...
- Weekly cap (RULE-20) is calculated, never stored or typed: 20 h in term, 48 h in holidays, 48 h after a verified completion letter. The 48-hour opt-out is Working Time Regulations, not a visa condition, so it removes the ceiling everywhere EXCEPT a student visa in term time — §4.4 gives "48 h — or no ceiling with a signed opt-out" for a student in a holiday range and for a graduated student, and the opt-out table reads "International student … Out of term — yes". A Mon–Sun week straddling term/holiday takes the lower cap.
