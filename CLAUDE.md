...
- Weekly cap (RULE-20 + `docs/scope/university-completion-letter-requirement.pdf`) is calculated, never stored or typed: 20 h in term (10 h below degree level), 48 h in holidays, 48 h from the COURSE COMPLETION DATE on a verified completion letter — not from the day it was verified, so a letter dated in the future lifts nothing yet. The 48-hour opt-out is Working Time Regulations, not a visa condition, so it removes the ceiling everywhere EXCEPT a student visa in term time; an under-18 cannot sign one, and a cancelled one restores 48 h from the end of the notice period. Right-to-work expiry outranks all of it — cap 0, and `canRoster()` is a per-shift hard stop because the week a visa expires still has workable days in it. A Mon–Sun week straddling term/holiday, the completion date, or the end of a notice period takes the lower cap.
...
- Client data path (ADR-0004): the client role gets a policy only on a table carrying no money and no worker personal data (today none at all — since `20260927160100` / ADR-0026 the client role holds no table policy and reads only through views; `001_rls_guard` pins the empty set). Everything else the portal shows comes from a `client_*` view that runs with owner rights, filters by `client_portal_visible()` in its own body and names its columns. Never add a client policy to `roles`, `shift_requirements`, `bookings` or `staff` — a view cannot take back a privilege the base table grants.
