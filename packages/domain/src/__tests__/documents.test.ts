...
describe('share codes', () => {
  it('accepts gov.uk’s spacing and case, and stores the nine characters', () => {
    expect(normaliseShareCode('w98 7zy-6xk')).toBe('W987ZY6XK');
    expect(formatShareCode('W987ZY6XK')).toBe('W98 7ZY 6XK');
  });

  it('refuses anything that is not nine letters and digits', () => {
    expect(normaliseShareCode('W98 7ZY')).toBeNull();
    expect(normaliseShareCode('W98 7ZY 6XK1')).toBeNull();
  });
});
