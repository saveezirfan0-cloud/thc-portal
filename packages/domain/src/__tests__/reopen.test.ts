...
/**
 * Who may reopen an ended booking row (§3.6, §3.4, §10.4; D33, ADR-0037).
 * booking_reopenable_by() in 20260930110100 is the same table, asserted
 * cause for cause in supabase/tests/661_additive_rounds_and_reopen.sql.
 */
const EXPECTED: Record<string, Reopener> = {
  ...
  ready_cutoff: 'person',
  self_cancel: 'never',
  // ADR-0039: a completed hand-over bars the event exactly as a self-cancel.
  handed_over: 'never',
...
describe('bookingReopenableBy', () => {
  ...
  it('of the causes a worker or the office brings about, only self-cancel and a hand-over bar for good (RULE-04, §3.6, ADR-0039)', () => {
