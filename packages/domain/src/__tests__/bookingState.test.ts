...
describe('the transitions the write paths make (§3.4, §3.5, §3.6, §10.4, RULE-15)', () => {
  it.each([
    ...
    ['apply_to_shift reopens a cancelled, not self-cancelled, row (ADR-0031)', 'cancelled', 'applied'],
    ['invite_worker reopens a closed offer (ADR-0031)', 'closed', 'invited'],
    ['invite_worker reopens a cancelled, not self-cancelled, row (ADR-0031)', 'cancelled', 'invited'],
