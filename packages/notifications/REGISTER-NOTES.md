...
| E3 | "activation + password + 'download the app'" | the Phase-0 seed body, `Set your password to start onboarding: {link}` | §8 and §2.7 both describe the email rather than quote it, and §2.7 also asks for the "download the app" CTA. **E3 is the only mandatory system email and has no copy anywhere in the scope** — it needs writing before activation can ship. |
