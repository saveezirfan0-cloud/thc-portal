...
test holds the set of codes to the two §8 tables (`SCOPE_CODES`). Three sends §8
...
self-cancel email (see "Missing from §8"), and E11, the Back Office / Client
Portal account invitation (ADR-0052). `src/inbox.ts` names the emails that go
to THC itself — every one whose recipients the register pins — for `/inbox`.
...
| E11 | not in §8 — ADR-0049 showed the `/users` set-up link rather than email it, because an email needed a register entry; THC approved adding it (ADR-0052) | extension E11: sender `admin`, to the login's own address (read by `queue_account_invite()`, never passed in), subject `Your THC {app} login`, body in E3's voice with the link, "works once and expires after 24 hours" and "reply to this email and we will send you a new one" | "24 hours" is `otp_expiry` in `supabase/config.toml`, and the test reads that file. Keyed `E11:invite:<user>:<n>`: a new link is a new email and withdraws an older unsent one; the same link twice is one. **Confirm the wording.** |
