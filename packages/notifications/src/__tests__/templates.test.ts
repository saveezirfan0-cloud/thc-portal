...
describe('notification register (§8)', () => {
  ...
  it('invents no code the scope or the completion letter requirement does not name', () => {
    expect([...Object.keys(TEMPLATES)].sort()).toEqual(
      [...PUSH_CODES, ...EMAIL_CODES, ...REQUIREMENT_PUSH_CODES, ...REQUIREMENT_EMAIL_CODES].sort(),
    );
  });
  ...
  it('exports SCOPE_CODES and REQUIREMENT_CODES as exactly the register, between them', () => {
    ...
    expect([...SCOPE_CODES, ...REQUIREMENT_CODES].sort()).toEqual(
...
describe('§8 copy is verbatim', () => {
  ...
  it('covers every code whose copy §8 quotes', () => {
    ...
    const unpinned = Object.keys(TEMPLATES).filter(
      (code) => !pinned.has(code) && !(REQUIREMENT_CODES as readonly string[]).includes(code),
    );
