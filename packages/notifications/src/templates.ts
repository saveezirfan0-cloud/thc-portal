...
export const TEMPLATES = {
  ...
  // invitation's own words (withdraw_booking(), 20260929110300).
  N10d: {
    code: 'N10d',
    channel: 'push',
    ...
    trigger:
      'The office withdraws an open invitation (manager presses Withdraw on an Invited row). Not in §8: N10b covers the Withdraw, but its copy says the worker was removed from a shift they had, which an invitee never did (ADR-0031)',
  ...
  N11b: {
    ...
    trigger:
      'Venue address or dress code changed on a booked shift (§3.5: "If the event time / date, venue address, or dress code changes → everyone booked must re-confirm … + push"). Not in §8: N11 is the only re-confirmation push §8 lists, and its copy is about the time (20260929110000 round, ADR-0031)',
    timing:
 ...
 * was the dress code (ADR-0031). Kept apart from SCOPE_CODES so the test can
