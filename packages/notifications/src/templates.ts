...
export const TEMPLATES = {
  ...
  // The account invitation (ADR-0038). ADR-0035 decision 3 showed the /users
  ...
  // queue_account_invite() (20260930120000) with the person's own address on
  ...
  E11: {
    ...
    trigger:
      'The office invites a Back Office or Client Portal login on /users, or issues it a new set-up link (ADR-0035, ADR-0038). Not in §8: §1.4 gives the office and the client a login by email and password, and §8 has no send for it, so THC approved the next free E-number',
    timing:
 ...
 * (ADR-0038). Kept apart from SCOPE_CODES so the test can still hold that
