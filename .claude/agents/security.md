...
2. **The client data path (ADR-0004):** the `client` role gets a policy only on a table carrying no money and no worker personal data (today none at all: since 20260927160100 / ADR-0026 the client role holds no table policy and reads only through views, and `001_rls_guard` pins the empty set). Everything else it sees is a `client_*` view with owner rights, filtering by `client_portal_visible()` in its own body and naming its columns. **Never** a client policy on `roles`, `shift_requirements`, `bookings`, `staff`. A view cannot take back a privilege the base table grants — so check the base grants, not only the view.
