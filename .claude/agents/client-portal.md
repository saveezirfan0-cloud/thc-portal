...
- Client role only; only their own events; read-only; **no money anywhere** (no rates, charges, margins). Data comes solely from `client_events_v` and `client_lineup_v`.
