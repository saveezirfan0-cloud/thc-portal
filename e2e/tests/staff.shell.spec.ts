...
test.describe('The shell around a compliant worker (§10.1)', () => {
  ...
  test('the bottom navigation is Shifts · Invites · Radar · Profile, in that order (ADR-0035)', async ({
    page,
  }) => {
  ...
  test('Documents lives under Profile, which is lit while it is open (ADR-0035)', async ({
    page,
  }) => {
...
test.describe('App lock — the four cases (§10.1)', () => {
  ...
  test('1b · a block whose document is no longer nameable says so (§4.3)', async ({ page }) => {
    ...
    // Profile — the home of Documents (ADR-0035) — is the only tab not
  ...
  test('a blocked worker can still reach the notification screen (§8)', async ({ page }) => {
    ...
    // and Profile — where Documents lives since ADR-0035, so the one tab a
