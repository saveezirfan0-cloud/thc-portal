 ...
 */
const FALLBACK = '/shifts';

describe('safeNext', () => {
  it('keeps a path on this origin, with its query and hash', () => {
    expect(safeNext('/shifts', FALLBACK)).toBe('/shifts');
    expect(safeNext('/shifts/42?tab=pay#top', FALLBACK)).toBe('/shifts/42?tab=pay#top');
    expect(safeNext('/documents', '/x')).toBe('/documents');
  });
  ...
  it.each([
    ...
    ['//evil.com/shifts', 'protocol-relative host with a path'],
    ...
    ['http://evil.com/shifts', 'absolute URL with a path'],
    ['javascript:alert(1)', 'a scheme'],
    ['shifts', 'a relative segment'],
    ['evil.com', 'a bare host'],
    [' /shifts', 'leading whitespace'],
  ...
  it('every value the browser would resolve off-origin is refused', () => {
    ...
    for (const next of ['/shifts', '/invites?x=1']) {
