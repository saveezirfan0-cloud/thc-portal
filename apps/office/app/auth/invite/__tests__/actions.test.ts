...
/**
 * Accepting a Back Office invitation — /auth/invite (ADR-0035 §3), with
 ...
 * mismatched password, a token that is not one. A Client Portal (or Staff
 * App) invitation opened here is signed straight out again and refused —
 * the Back Office admits `app_metadata.role = admin` and nothing else.
 */
...
describe('the link lands without a session', () => {
  it('the middleware serves /auth/invite to a signed-out visitor rather than sending them to /login', async () => {
    const res = await middleware(
      new NextRequest(`http://127.0.0.1:3000/auth/invite?token=${TOKEN}`),
    );
