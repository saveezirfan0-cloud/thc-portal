...
import { RtwReportUpload } from './EvidenceUploads';
import {
  ApproveModal,
  ConfirmVerifyModal,
  RejectModal,
...
import type { ConfirmedConditions } from './ReviewModals';
import type { WhoFilter } from './queue';
import { rtwDateRule } from './rtw';
import type { ActionResult, QueueRow } from './types';

/** The Verify's conditions, addressed to the worker they belong to. */
function withStaff(row: QueueRow, conditions: ConfirmedConditions) {
  return Object.keys(conditions).length ? { staffId: row.staff_id, ...conditions } : undefined;
}
...
export function ReviewTab({
  ...
  const verify = (row: QueueRow) => {
    ...
    if (row.kind === 'document' && needsConfirmWindow(row)) {
      setChecking(row);
      return;
    }
    run(row.item_id, () =>
  ...
  return (
    <section className="stack" aria-label="Needs review">
      ...
      <Alert tone="cyan">
        ...
        off the gov.uk report is confirmed, and NI evidence verified before the NI number was
        entered, once it has been.
      </Alert>
              ...
              () =>
                confirming.kind === 'rtw_date'
                  ...
                  : verifyDocument(
                      ...
                      withStaff(confirming, conditions),
                    ),
              () => setConfirming(null),
            )
          }
      ...
      {checking ? (
        <ConfirmVerifyModal
          item={checking}
          busy={pendingId === checking.item_id}
          onClose={() => setChecking(null)}
          onVerify={(conditions) =>
            run(
              checking.item_id,
              () => verifyDocument(checking.item_id, {}, withStaff(checking, conditions)),
              () => setChecking(null),
            )
          }
        />
  ...
  return (
    <tr>
      ...
      <td>
        ...
        {row.kind === 'ni_check' ? <Pill tone="amber">compare NI number</Pill> : null}
        <span className="sub">
          {documentLine(row)}
        ...
        {/* The manual path's report (ADR-0018): the automated check stores its own. */}
        {row.item_type === 'share_code_report' &&
        (row.kind === 'document' || row.kind === 'rtw_date') &&
        row.gov_report_path === null &&
        !row.rtw_check_report_path ? (
          <div className="sub">
            <RtwReportUpload docId={row.item_id} staffId={row.staff_id} />
          </div>
        ) : null}
      </td>
