...
export function RateCard({
  ...
  const unlisted = roles.filter((role) => !rows.some((row) => row.role_id === role.id));
        ...
        <>
          ...
          <Select
            ...
            onChange={(event) => {
              ...
              setAdding('');
              // A new row starts at the role's own base pay, which is a
              // 0% margin — visibly wrong rather than plausibly wrong, so
              // nobody leaves it unset by accident.
              const role = roles.find((entry) => entry.id === id);
              if (role) run(() => addRole(clientId, role.id, role.pay_rate, []));
            }}
          >
                    ...
                    <td>
                      <InputRow>
                        ...
                        <Input
                          ...
                          value={rate}
                          inputMode="decimal"
                          aria-label={`Charge rate for ${row.role_name}`}
