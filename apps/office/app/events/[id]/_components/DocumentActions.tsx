...
const LABEL: Record<Kind, { send: string; download: string; noun: string }> = {
  ...
  signout: {
    send: 'Send sign-out timesheet',
    download: 'Download sign-out timesheet',
    noun: 'sign-out timesheet',
  },
};

/** Which pair the header carries, per state (event-board.html:128, 243, 307). */
export function documentKinds(status: EventStatus): Kind[] {
  if (status === 'cancelled') return [];
  // Completed: the sign-out timesheet only, Send primary — the allocation
  // sheet's moment has passed. Before and during: the allocation sheet,
  // which can still be sent or downloaded mid-event (§11.3).
  return status === 'completed' ? ['signout'] : ['allocation'];
}

/**
 * §11.4 on the event page: "Send allocation sheet" (an action) + "Download"
 * (a PDF, for WhatsApp) while the event is Upcoming or Ongoing; "Send
 * sign-out timesheet" + "Download sign-out timesheet" once Completed — it
 * is filled from check-in/out, so before that it would be the allocation
 * sheet again.
 *
 ...
 */
export function DocumentActions({ eventId, status }: { eventId: string; status: EventStatus }) {
  const kinds = documentKinds(status);
