...
| `/users` | Users & access: every login by app, Invite a Back Office (with office role) or Client Portal user — the one-time set-up link is emailed (E11, ADR-0052) and also shown to copy — switch a login off/on, new invite link. States: empty tab, search with no match, invite link shown once, switch-off needs a reason, own row has no Switch off | — (ADR-0049) | 1.4, 1.7 | platform |
