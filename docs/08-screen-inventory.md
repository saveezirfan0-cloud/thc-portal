...
| `/client` | Event list. Additions (ADR-0034, 0035, 0038): Next up strip (happening now · next · hidden), venue + From/To filters, per-role fill line, feedback nudge, signed-timesheet status, "N of M arrived" while ongoing, "(+1 day)" on overnight windows; empty states: no events yet · nothing in this tab · no match for the filters (Clear filters) | `client/events.html` | 11.1 | client-portal |
| `/client/events/:id` | Event page + feedback popup. Additions: Add to calendar (.ics, hidden when cancelled, ADR-0035), "(+1 day)" on overnight windows (ADR-0035), "N of M arrived" in the header and per role while ongoing (ADR-0038) | `client/event.html` | 11.2, 11.5 | client-portal |
