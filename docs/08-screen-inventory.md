...
| `/staff/:id` | Profile (Overview/Documents/Client qualification/Shifts/Feedback/Availability). Shifts: a `handed_over` cancel cause reads "Handed over (offered up)", never "Self-cancelled" (ADR-0039). Documents: Verify / Reject on every document or Yes declaration this worker has on Needs review (Confirm date on an `rtw_date` row) — the `/compliance` actions and dialogs (`compliance/ReviewDialogs.tsx`), not a copy; none on a Rejected/Removed worker (§4.1) | `backoffice/staff-profile.html` | 9.6 | directory, compliance |
| `/staff/requests` | Change requests (docs/18 §3, `office_profile_change_requests` / `office_decide_profile_change`) — Pending (oldest first; now → requested side by side, signed photo URLs, evidence link, note; **Approve** with the right-to-work evidence tick for a name, asked again on the server; **Reject** with a required reason "shown to the worker") · Decided tab (decision, reason, the manager or "— the worker" / "GDPR removal", UK stamps) · empty state. `/staff` carries "Change requests (N)" and the Staff menu item the same count (`navCounts.ts`) | `backoffice/change-requests.html` | 10.1, 9.6 · ADR-0038 | directory |
| `/staff/:id` (Availability tab) | Read-only Availability tab (docs/18 §1, `office_staff_unavailability`): the worker's unavailable entries for the next 8 weeks, UK time — When (UK) · Length (23/25 h across a clock change) · Repeats ("weekly · N (to …)") · Overlaps a booking (confirmed, matched on the role section's window, RULE-18) · Added. States: rows · empty ("Nothing marked in the next 8 weeks") · read failure | `backoffice/change-requests.html#cr=availability` | 9.6 · ADR-0036 | directory |
| `/staff/:id` (Overview cards) | Overview (docs/18 §2, §3, §5) carries an **Emergency contact** card (Add / Edit / Clear, audited by field name, "Not provided" when empty, none on a removed profile; office-only, never on a client document), a **Referrals** card with the "Referred by" line, the worker's code and everyone who applied with it ("Deleted account #id" for a removed person), and above the tabs a pending change-request banner whose **Review** opens the `/staff/requests` decide dialog | `backoffice/change-requests.html#cr=overview` | 9.6, 1.7 · ADR-0037, ADR-0038, ADR-0040 | directory |
...
| `/radar/offers/:id` | **Built** (docs/18 §4). One offered shift ("Up for grabs") and **Take this shift**; refusals reuse Radar's copy. `/radar` gains an "Up for grabs" group (RULE-17 visibility, offerer never shown) | `staff/offer-shift.html` | 10.4, 3.4 · ADR-0039 | scheduling |
| `/shifts/:id` (offer states) | **Built** (docs/18 §4). **Offer this shift** (> 72 h, auto-assign on) → confirm → "Offered · open until …" + **Withdraw offer**; inside 72 h **Ask the office for cover** → "Cover requested"; `handed_over` static line; "Offered" chip on the `/shifts` card | `staff/offer-shift.html` | 10.4, RULE-04 · ADR-0039 | scheduling |
...
| (tab) Profile hub additions | (docs/18 §1, §2, §5; ADR-0036/0037/0040 proposed). Profile gains rows "Availability — Days you can't work" and "Refer a friend" (compliant only); Profile details carries an amber "Emergency contact not set" subline while empty (a nudge, not a lock) | `staff/availability.html`, `staff/refer.html` | 10.1 · ADR-0036, ADR-0037, ADR-0040 | staff-pwa |
...
| `/profile/details` (Emergency contact) | (docs/18 §2, ADR-0037 proposed). Emergency contact section — name, relationship, phone with the international picker, Save, Remove | `staff/request-change.html#emergency` | 10.1, 1.7 · ADR-0037 | staff-pwa |
| `/profile/details/request` | (docs/18 §3, ADR-0038 proposed). Request a change — `?kind=name` (first/last, evidence upload, note) or `?kind=photo` (camera capture); pending status line, withdraw, "Not changed: {reason}" + Request again | `staff/request-change.html` | 10.1 · ADR-0038 | staff-pwa |
| `/profile/refer` | (docs/18 §5, ADR-0040 proposed). Refer a friend — link, Share, Copy, QR, "N people have applied with your link"; no reward copy. Compliant only | `staff/refer.html` | 2.1, 10.1 · ADR-0040 | staff-pwa |
