...
| `/login/verify` | Two-step sign-in code step: after email + password, a login with a verified authenticator types its 6-digit code; `next` carried through. States: code form, wrong / expired code, too many attempts, no session (→ `/login`), already verified (→ `next`), a factor this screen cannot challenge, Not you? Sign out | — (ADR-0051) | 1.4 | platform |
...
| `/users` | Users & access: every login by app, Invite a Back Office (with office role — owner, manager, scheduler or read-only viewer) or Client Portal user — the one-time set-up link is emailed (E11, ADR-0052) and also shown to copy — switch a login off/on, new invite link, change office role, Reset two-step on a Back Office row with two-step on (ADR-0054). States: empty tab, search with no match, invite link shown once, switch-off and two-step reset need a reason, own row has no Switch off / Change role / Reset two-step | — (ADR-0049, ADR-0054) | 1.4, 1.7 | platform |
| `/activity` | Activity log: `audit_log` newest first with who / what / record / details, filters by area, person, text and period, paging by 50. States: empty period, no match, System entries | — (ADR-0049) | 1.7, 1.8 | platform |
| `/activity/export` | Activity log as CSV (the "Export CSV" button on /activity): the same filters from the query string (not the page), newest first, BOM, When (UK time) · Who · Action · Area · Record · Details; stops at 10,000 rows and says so on the last line; 401/403 without an admin session | — (ADR-0049) | 1.7, 1.8 | platform |
| `/inbox` | Inbox: the emails the platform sent to the office and payroll (E5–E10, CL3–CL6, the Monday payroll email) from `notification_outbox`, newest first — what (type + rendered subject), about whom (worker linked to their profile, or event / payroll week), to, queued (UK time), status with the sent/failed stamp and the error. Filters by type, status and period, paging by 50. Read-only. States: empty period, no match, failed-in-period banner, queued / held (no keys) / retrying / sent / failed, no Supabase project | — (ADR-0052) | 8, 1.8 | notifications |
| `/auth/invite` (Back Office and Client Portal) | Set up your login: an invitee chooses a password; the one-time token is spent on submit, not on page load; refuses a token for the other app | — (ADR-0049) | 1.4, 10.2 | platform |
| `/account` | My profile: name, job title, phone; sign-in email change (confirmation link); password change (current password checked); sign out other devices; appearance | — (ADR-0049) | 1.4, 10.2 | platform |
