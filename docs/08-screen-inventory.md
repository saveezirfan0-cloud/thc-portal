...
| `/profile/availability` | **Planned** (docs/18 §1). Availability calendar — entries by week, **Add** sheet (day / range / time window "(UK time)", repeat weekly ≤ 26), conflict warning over a confirmed shift, delete one or the series, empty state. Only when `appLock() === 'none'` | `staff/availability.html` | 9.6, 10.1 · ADR-0036 | staff-pwa |
