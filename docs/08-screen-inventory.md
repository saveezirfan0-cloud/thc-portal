...
| `/activate`, `/activate/:token`, `/activate/done` | Set password → Activated → Install the app. The token is spent on SUBMIT, never on page load — mail scanners prefetch links. | `public/activate.html` | 2.7 | onboarding, staff-pwa |
| `/privacy` | Privacy notice (the `/apply` consent links here) | — | 1.7 | platform |
...
| `/compliance/export` | Audit export (route handler, CSV) | — | 4.5 | compliance |
...
| `/reports/export` | CSV export, one row per shift (route handler) | — | 9.9 | reports |
| `/api/documents/:eventId`, `/api/documents/:eventId/send` | Allocation sheet · Sign-out timesheet: download and send (route handlers) | `client/timesheet.html` | 11.3–11.4 | reports |
...
| `/offline` | Service-worker fallback when a navigation cannot be served | — | 10.5 | staff-pwa |
| `/onboarding`, `/onboarding/:step` | Wizard, 11 steps. The route is `:step`, not `/1`…`/11`. | `staff/onboarding-1.html`, `-2`, `-3` | 10.3, 2.5–2.11 | onboarding |
| `/documents` | Documents hub / tab | `staff/onboarding-3.html`, `staff/documents.html` | 10.4 | compliance |
| `/documents/upload/:docType` | Upload or re-upload one document | `staff/documents.html` | 10.4, 2.6 | compliance |
| `/documents/completion-letter` | Course completion letter | `staff/documents.html` | 4.5 | compliance |
| `/documents/opt-out` | Sign or cancel the 48-hour opt-out | `staff/documents.html` | 4.4 | compliance |
| `/documents/declare` | Declare a conviction in employment | `staff/documents.html` | 10.7 | compliance |
...
| `/profile`, `/profile/details`, `/profile/security`, `/profile/payments` | Profile sheet, then Profile details, Security, Payment information. **Nested under `/profile`** — this page used to say `/security` and `/payments`, and the app has never served those. | `staff/profile.html` | 10.1 | staff-pwa |
...
| `/client/events/:id/document` | The latest allocation sheet / timesheet PDF (route handler) | `client/timesheet.html` | 11.3 | reports |
