...
| `/client` | Event list. Additions (ADR-0049, 0035, 0038): Next up strip (happening now · next · hidden), venue + From/To filters, per-role fill line, feedback nudge, signed-timesheet status, "N of M arrived" while ongoing, "(+1 day)" on overnight windows; empty states: no events yet · nothing in this tab · no match for the filters (Clear filters) | `client/events.html` | 11.1 | client-portal |
