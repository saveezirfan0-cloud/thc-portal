...
| `/documents` | Documents hub — reached from the Profile tab since ADR-0041 (URL unchanged for §8 deep links) | `staff/onboarding-3.html`, `staff/documents.html` | 10.4 | compliance |
...
| `/radar/offers/:id` | **Built** (docs/19 §4). One offered shift ("Up for grabs") and **Take this shift**; refusals reuse Radar's copy. `/radar` gains an "Up for grabs" group (RULE-17 visibility, offerer never shown) | `staff/offer-shift.html` | 10.4, 3.4 · ADR-0045 | scheduling |
| `/shifts/:id` (offer states) | **Built** (docs/19 §4). **Offer this shift** (> 72 h, auto-assign on) → confirm → "Offered · open until …" + **Withdraw offer**; inside 72 h **Ask the office for cover** → "Cover requested"; `handed_over` static line; "Offered" chip on the `/shifts` card | `staff/offer-shift.html` | 10.4, RULE-04 · ADR-0045 | scheduling |
| `/invites`, `/invites/:id` | Invites | `staff/invites.html` | 10.4, 3.4 | scheduling |
| (tab) | Profile tab — the §10.1 sheet as a screen, Documents first, Edit profile (ADR-0041) | `staff/profile.html` | 10.1 | staff-pwa |
...
| (tab) Profile hub additions | (docs/19 §1, §2, §5; ADR-0042/0043/0046 proposed). Profile gains rows "Availability — Days you can't work" and "Refer a friend" (compliant only); Profile details carries an amber "Emergency contact not set" subline while empty (a nudge, not a lock) | `staff/availability.html`, `staff/refer.html` | 10.1 · ADR-0042, ADR-0043, ADR-0046 | staff-pwa |
| `/profile/availability` | (docs/19 §1, ADR-0042 proposed). Availability calendar — entries by week, **Add** sheet (day / range / time window "(UK time)", repeat weekly ≤ 26), conflict warning over a confirmed shift, delete one or the series, empty state. Only when `appLock() === 'none'` | `staff/availability.html` | 9.6, 10.1 · ADR-0042 | staff-pwa |
| `/profile/details` (Emergency contact) | (docs/19 §2, ADR-0043 proposed). Emergency contact section — name, relationship, phone with the international picker, Save, Remove | `staff/request-change.html#emergency` | 10.1, 1.7 · ADR-0043 | staff-pwa |
| `/profile/details/request` | (docs/19 §3, ADR-0044 proposed). Request a change — `?kind=name` (first/last, evidence upload, note) or `?kind=photo` (camera capture); pending status line, withdraw, "Not changed: {reason}" + Request again | `staff/request-change.html` | 10.1 · ADR-0044 | staff-pwa |
| `/profile/refer` | (docs/19 §5, ADR-0046 proposed). Refer a friend — link, Share, Copy, QR, "N people have applied with your link"; no reward copy. Compliant only | `staff/refer.html` | 2.1, 10.1 · ADR-0046 | staff-pwa |
...
> **Planned rows · docs/19 (ADR-0042–0046, proposed — awaiting THC).** Rows marked
