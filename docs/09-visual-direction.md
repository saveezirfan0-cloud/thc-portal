...
**Status:** proposed to THC (ADR-0003). The Scope of Work §1.6 marks the visual system as STRICT, so this is a recorded change request, not a silent drift. Both looks ship from the same tokens; the switch in the wireframes' top bar shows either at any time (`Warm` / `Scope §1.6`, `Dark` / `Light`).
