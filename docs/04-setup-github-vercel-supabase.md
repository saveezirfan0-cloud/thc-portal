...
- Auth: enable Email provider, disable sign-ups (workers are invited; admins/clients are created by an admin), set Site URL per app, add redirect URLs for `/activate` and each app's `/auth/callback` (Staff App, Back Office and Client Portal all send a reset link that lands there, then on `/reset`).
