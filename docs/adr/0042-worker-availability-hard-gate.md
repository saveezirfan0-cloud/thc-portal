...
row). Plan: `docs/19-staff-features-plan.md` §1. THC: Q9, Q10 (`docs/15-open-questions.md`).
...
- **What never changes.** §6's weights; the existing hard gates; open invitations are
  ...
  (ADR-0004/0026; `001_rls_guard`'s empty set holds, pgTAP 700 adds a `pg_depend`
  check). No PDF, report or payroll export reads it. No notification is added.
- GDPR removal deletes the rows (trigger `staff_removed_purge_additions`, pgTAP 702).
- pgTAP 700–702 (Phase 0), 705 (worker RPCs), 706 (`invite_worker` refuses
