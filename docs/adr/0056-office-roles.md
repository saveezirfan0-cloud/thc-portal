...
| `roles`, `client_rate_cards` | tables | **writes** refused (restrictive INSERT / UPDATE / DELETE), so `create_role` / `update_role` / `delete_role` and the three rate-card functions, which run as the caller, change nothing. Reads of the rows open; the rate **columns** not selectable by any API session since ADR-0061. |
| `shift_requirements.pay_rate` / `charge_rate` | table | a trigger on direct writes: a new section carries the role's `pay_rate` and the client's rate-card charge (0 when the client has none — the builder's own default); an edit keeps the rates unless the role changes, when the new role's catalogue rates apply. This refused with `rates_need_finance`; since ADR-0061 it sets the rates instead, and the columns are not selectable. |
...
1. ~~**Rate columns on rows scheduling needs.** `roles.pay_rate`, `shift_requirements.pay_rate` / `charge_rate`, `client_rate_cards.charge_rate` and the copies on `payable_shifts_v` stay readable to every Back Office login through the API.~~ — **closed by `20261001203000` (ADR-0061)**: no API session may select those four columns at all; owners and managers read them through `shift_rates_v`, `role_rates_v` and `rate_card_rates_v`, which return no row to a scheduler, and `payable_shifts_v` carries them as NULL to a scheduler. The office RPCs that returned a worker's base rate for any worker (`staff_bookings(p_staff)`, `staff_open_shifts(p_staff)`, `check_out`) withhold it from a scheduler too.
2. ~~**The event builder and event board show those rates on screen** to a scheduler.~~ — **closed by ADR-0061**: no rate, charge, margin or forecast money is drawn for a scheduler, none is sent on save, and a scheduler's section is given the catalogue rates by the database (set, no longer refused — a refusal that depends on the value typed would reveal the rate).
3. ~~**Client card** (`/clients/[id]`): margins show "—" and the rate card's Add / Edit / Remove role controls are drawn and refused.~~ — **closed by ADR-0061**: for a scheduler the rate card is roles and dress codes only ("Rates hidden for your role"), with no controls, no margin tile and no margin column.
...
- `20261001200100_office_roles.sql`, `supabase/tests/741_office_roles.sql` (136 assertions); `001_rls_guard` pins the fifteen restrictive policies exactly (assertions 10, 10b).
...
- `20261001200700` rewrites every `public` policy that called `current_app_role()` or `office_can()` bare to `(select …)`, so each runs once per statement instead of once per row (the stricter `current_app_role()` of `20261001200500` had tripled per-row cost). The rule is unchanged; pgTAP 747 fails if a bare call comes back.
