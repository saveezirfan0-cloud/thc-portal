...
**§11.3 unchanged.** Plan: `docs/19-staff-features-plan.md` §2. THC: Q11, Q12
...
- GDPR removal deletes the row (`staff_removed_purge_additions`, pgTAP 702). A leaver
  (`inactive`) can still read it but not write it; a removed worker is refused.
- pgTAP 700 (matrix + no `client_*` dependency), 702, 710 (worker save → read → clear,
  bad phone refused, no cross-worker access), 711 (office save audited, non-admin
  refused).
