...
**Status:** Accepted, 25.09.2026 — built and tested; **switched off until the keys exist and the live test is done** (no provider since ADR-0041) (OWNER-TODO §8) · **Amended by [ADR-0041](0041-rtw-check-admin-confirms-no-provider.md) (26.09.2026):** no provider (gov.uk is the only route) and every result waits for an admin's Verify or Reject after a photo comparison; §1–§2's automatic verify/reject stays behind `settings.rtw_check.admin_confirms = false`
