# ADR-0060 · A read-only viewer role, resetting two-step from /users, and workers' activation links fenced

**Status:** Accepted (product owner approved the viewer role) · **Builds on:** ADR-0055, ADR-0056, ADR-0057, ADR-0058 · **§1.4, §1.7, §2.7, §2.8 (E3), §8, §10.2**

## Context

Three pieces of unfinished business from the office-roles work:

1. ADR-0055 sketched a fourth office role, **viewer** — someone who can look at everything and change nothing (an auditor, the accountant, a new starter shadowing the office). ADR-0056 left it out. THC has now approved it.
2. ADR-0057 left the lost-phone recovery to someone with Supabase dashboard access, with no audit row. Its follow-up 2 was a reset from `/users`.
3. ADR-0056's closing note: E3 rows in `notification_outbox` carry a worker's one-time activation link, and every Back Office login could read them (`admin_read`). Whoever holds the link sets that worker's password before they do. E11 was fenced by `20261001200600`. E3 was not.

## Decision

### 1 · The viewer role (`20261001201000`, `20261001201100`)
...
- **Allow-list**, pinned by pgTAP 750:
  ...
  - `office_saved_views`: a viewer's own filter chips on /events (ADR-0059, `20261001202000`). It is own-row only, and a preference rather than office data.
...
### 2 · Reset two-step from /users (`20261001201200`)
...
- **Redaction**: `redact_finished_invite_link()` (20261001200600) now covers E3 as well as E11. Once a row is sent, or has failed for good, `payload.link` is removed and `linkRedacted: true` is set. An **unsent** E3 keeps its link, because the drain sends it and a resend points it at the newest token. `installLink` (/install) and `name` stay; they are not secrets. Rows already finished are redacted by the migration.
...
- New: `20261001201000_viewer_role_enum.sql`, `20261001201100_viewer_role_read_only.sql`, `20261001201200_two_step_reset_and_activation_links.sql`; pgTAP `750_viewer_role`, `751_two_step_reset`, `752_activation_links_owner_only`.
