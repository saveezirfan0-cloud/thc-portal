...
`docs/18-staff-features-plan.md` §3. THC: Q13, Q14, Q21 (`docs/15-open-questions.md`).
...
Bodies are in `docs/18` §3 and `packages/notifications`. Every send is a
...
- GDPR removal anonymises proposed names and withdraws pending requests (pgTAP 652).
- pgTAP 650, 651 (vectors), 652, 665 (second pending refused, withdraw, foreign or
  missing photo path refused, name equal to current refused), 666 (approve name → RC2 +
