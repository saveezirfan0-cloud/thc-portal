...
4. **A "Next up" strip above the list** (`nextUp`). A running event wins: "Happening now · Gala Dinner · until 23:30 UK · 13 of 17 confirmed". Otherwise the soonest upcoming event: "Next · Gala Dinner · today 07:00 UK · …". The strip links to the event page and is not drawn when nothing is upcoming or ongoing. Cancelled and completed events never appear in it. The time is one UK time with the " UK" suffix (§1.8), because the strip is only a pointer; the event page shows the full dual-zone window. "Today" and "tomorrow" are judged on the UK calendar. The strip ignores the tab and the filters: it answers "what is next for me", not "what is in this view".
...
- **`e2e/tests/client.portal.spec.ts` needs a one-line update.** It looks for a button named "Upcoming & ongoing" in its tab test; the label is now "Upcoming".
- **`docs/08-screen-inventory.md` should list the new `/client` states:** Next up (live / next / none), the three empty states, and the filtered list.
