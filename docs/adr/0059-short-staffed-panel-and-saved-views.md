# ADR-0059 · The dashboard's short-staffed panel, and saved views on Scheduling
...
3. **Saved views follow the manager across devices** (updated 26.09.2026, `20261001202000`). They were first kept per browser in `localStorage`; they now live in `office_saved_views` (id, owner = `auth.uid()` by default, scope `'events'`, name 1–60 characters, query jsonb, created_at, updated_at; unique on owner + scope + `lower(name)`).
