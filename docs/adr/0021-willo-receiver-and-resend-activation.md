...
## 2026-09-26 · The create is idempotent (migration `20260926100200`, pgTAP 522)

**Closes** the known edge in §3 above (docs/14 §4, "New from the 24.09 wave"): a candidate created in Willo whose local link then failed transiently was created again by the next lease, and Willo sent a second E1. The receiver (§2) and its replay safety are untouched.

### What changed

The order of the sweep is now the whole rule, and every step of it lives on the `staff` row rather than only in `audit_log`:

1. **Lease on the row.** `willo_invite_due()` sets `staff.willo_create_claimed_at` inside the same `for update skip locked` pass that picks the row, so the per-minute schedule and the nudge can never both hold one candidate. The lease lasts as long as the retry backoff already did (5 min after the first attempt, doubling, capped at 6 h): a claim is not offered again before it, answered or not. That is longer than a sweep can run — the function claims at most 20 rows, gives Willo 10 s each, and stops attempting after 120 s, releasing the rest as a plain failure (`deferred` in `job_runs`) so the backoff, not the stale budget, decides when they are next offered.
2. **The key is written first, alone.** The moment Willo answers 2xx, the function calls `willo_create_recorded()`, which stores the key in `staff.willo_created_candidate_id` with no status guard and nothing else in the statement, retried in-process (3 tries, 0.5 s doubling) on a transient database error. It is idempotent; the **first** key wins and any second is audited as `willo_invite_duplicate`, so a duplicate in Willo is visible instead of silently adopted.
3. **Only the link is retried.** `willo_link_candidate()` (restated from `20260923110000`) is the guarded step: it links, closes the lease and refuses a key that differs from the recorded one (`willo_candidate_id_mismatch`). A row holding a recorded key but no `willo_candidate_id` comes back from the next sweep as `kind = 'link'` with that key, and the function runs only the link. A row with a key is never offered as `kind = 'create'`.
4. **An unanswered lease is bounded.** A claim that is neither linked nor released by the time its lease ends (the function died mid-flight, or reported the outcome as unknown) is retried — a *stale retry* — at most **3 times** per onboarding period, or since the office's last Retry. After that the row is flagged (`willo_create_stuck_at`) with a `willo_invite_failed` audit row saying `no answer from create attempts in a row (lease expired 3 times); check Willo for <email> before retrying`, and leaves the queue. It is **never re-created blindly**.
5. **The function says how an attempt ended.** `willo_invite_failed(p_staff, p_error, p_outcome)` — the two-argument overload is dropped — takes `'failed'` (Willo refused, or was never reached: DNS, connect refused, TLS; the lease is released and the backoff applies), `'unknown'` (a timeout, a connection dropped after the request went out, a 2xx whose key could not be read, or a key that could not be written: the lease is **kept**, so the row takes the bounded stale path), or `'stuck'` (a link refused for good, e.g. a key already linked to somebody else: flagged now). A `not_awaiting_interview` on the link (the candidate was rejected or removed meanwhile) is `'failed'`; they are out of the queue by status, and a later Reset is a new interview by §2.12.

### The office surface

The surface is **`onboarding_candidates_v`**, not an outbox row: the office already reads this card for Willo tracking, and the §8 register has no office-facing onboarding template to send. The view (restated from `20260924160000`, its latest definition, with four columns appended) now carries `willo_create_stuck_at`, `willo_created_candidate_id` (the key to look for in Willo when a link could not complete), `willo_create_attempts` (claims this period) and `willo_create_last_error`.

`willo_invite_retry(p_staff)` is the office's remedy once it has looked in Willo: admin only (`assert_office_caller`), refused for a candidate who is not stuck or already in Willo, audited as `willo_invite_retry` with the manager as actor. It clears the flag and the lease; the next `willo-invite` run retries the link if a key was recorded, otherwise the create, with a fresh stale budget. It does not nudge the function itself — the schedule is the safety net and runs every minute once enabled.

**Deviation from the wireframe (follow-up, `apps/office`):** `wireframes/backoffice/candidate.html` has no "Willo invitation stuck" state. The candidate profile and the Interview requested column should show it (the flag, attempts, last error, the recorded key as a link when `settings.willo_review_url_template` is set) with a **Retry** outline button calling `willo_invite_retry`. Not built in this change.

### What forgets an attempt

`staff_willo_create_reset` (a small `before update of status, removed_at` trigger beside `staff_status_guard`, which is not restated — docs/14 §8) nulls the lease, the recorded key and the flag on entry to `interview_requested` (a Reset: §2.12 step 1, a fresh interview is due) and on removal (§1.7, as `willo_candidate_id` already is). The three columns are granted to `anon, authenticated` by name (`20260923210000`'s rule; none carries personal data or money), so the view keeps working.

### Residual risk, stated

- If the database is unreachable for both the key write **and** the outcome write, the key exists only in the function's log (`created <key>, not recorded`). The lease stands, so the next sweep retries after the backoff; that retry **is** a second create. It is bounded (at most 1 + 3 creates per period) and ends with the office being told whom to look for. Willo's own de-duplication by email, if it has one, would make it a no-op; to be confirmed with the real API (§1).
- A connection dropped *after* the request went out is classified by message (`(Connect)`, `dns error`, `tcp connect`, `ECONNREFUSED`, `ENOTFOUND`, `certificate`, `tls` → never connected → `'failed'`; anything else → `'unknown'`). Deno's fetch error text is the only evidence there is; a misclassified connect failure costs one stale retry, never a blind loop.
- The lease timing is the same clock as before, so pgTAP 482's timings hold unchanged; 522 holds everything above (claiming is exclusive under the lease, a row with a key is never offered as a create, the 3-then-stuck path, the view, Retry, grants, the reset and removal trigger).

### Deploy

- `supabase functions deploy willo-webhook --no-verify-jwt` (the function reads `kind` / `willo_candidate_id` from `willo_invite_due` and calls `willo_create_recorded`; the old function against the new migration would still work but would not persist the key first).
- Nothing new to set. `deno check` (Deno 2.9.6) passes on the function with `https://esm.sh/@supabase/supabase-js@2.45.4` mapped to `npm:@supabase/supabase-js@2.45.4` through an import map, because the build sandbox's proxy refuses esm.sh; the code is unchanged by the mapping.
