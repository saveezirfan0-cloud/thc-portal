...
4. **Change password is the one input.** It is an authentication feature: it changes how this person signs in, not any business data THC keeps about the account. So it does not break §11.1's "no editing whatsoever", which is about the portal's business content. The server action:
