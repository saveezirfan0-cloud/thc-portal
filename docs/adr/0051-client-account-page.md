# ADR-0051 · A "Your account" page in the Client Portal
...
3. **The company and the recipients come from a new view, `client_account_v`** (`20261001100000`). It uses the `client_company_v` shape exactly: owner rights, `security_barrier`, `c.id = current_client_id() and client_portal_visible(c.id)` in the view body, exactly two named columns (`name`, `contact_emails`), and `select` granted to `authenticated` only (anon revoked). It returns the caller's own row and nothing else. It carries no terms (`pays_breaks`, `pays_buffer`), no contact name or phone, no money and nothing about workers. **No policy is added to `clients`.** ADR-0026's empty set of client table policies still holds. `supabase/tests/606_client_account_view.sql` pins all of this.
4. **Change password is the one input, and it is an exception to §11.1.** §11.1 says "Read-only — no editing whatsoever", and a password change is a write (to Supabase Auth) made from inside the portal. It is therefore recorded here as a deviation from the scope, agreed with the client on 25.09.2026, and the portal's second write after §11.2 feedback. The reasoning for accepting it: it changes how this person signs in, not any business data THC keeps about the account, and without it a client's only way to change a password is the forgotten-password email. The server action:
   ...
   **The re-verification is throttled per account** (security review L2, `20261001100200`). Every attempt reaches Supabase Auth from Vercel's addresses, so Supabase's per-IP limit does not bind someone guessing the current password from a left-open session, and when it trips it locks out every other portal user instead. Before `signInWithPassword`, the action calls `password_check_allowed()` on the caller's own session: past **5 failed checks in 15 minutes** it returns "Too many attempts" without calling `signInWithPassword`, and if the RPC errors it fails closed with the generic failure message. After a wrong current password it calls `record_password_check_failure()`. Supabase's own rate limit and network errors are not counted. The limit is purely time-based: a correct password does not clear the count. The failures live in `private.password_check_failures`, the `apply_caller_hits` shape (not in `public`, RLS on with no policy, no API-role grant). Both functions are security definer, key on `auth.uid()` only (a null uid is refused and records nothing), and are closed to anon and PUBLIC. Each write trims the caller's rows older than a day. `supabase/tests/608_client_password_attempts.sql` pins this. Known residual: checking and recording are two calls, so a burst of parallel requests can all pass the check before any of them is recorded. The limit applies again from the next round trip.
