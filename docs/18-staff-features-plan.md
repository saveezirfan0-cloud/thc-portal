...
**pgTAP:** 650 · 652 GDPR revokes the removed worker's code; referral rows kept, referrer reads "Deleted account #id" · 680 (B) minted once and stable, compliant only, summary is a count · 681 (D) valid code records a row for both `candidate_created` and `returning_applicant`; bad/revoked/self code records nothing and **the application still succeeds**; throttles untouched; anon path records nothing; response identical with or without a code.
