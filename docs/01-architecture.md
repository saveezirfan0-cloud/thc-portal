...
| AI document extraction (§2.6) | `DocumentExtractor` interface in `apps/staff/app/onboarding/extractor.ts` (server-side, called from both upload paths through `apps/staff/lib/extract.ts`) — no separate Edge Function; provider = **Anthropic's Claude** in `extractors/anthropic.ts` (ADR-0031: replaces the scope's Gemini, accepted by the product owner, **awaiting THC's confirmation**), **switched off** until `ANTHROPIC_API_KEY` is set on the Staff App's Vercel project — until then `documentExtractor()` returns `null`. The write path: `record_document_extraction()` (service role only) pre-fills `ai_extracted`, `ai_confidence`, dates and `needs_manual_review` against `settings.ai_confidence_threshold`; a failed or doubtful read arrives flagged for manual review. **Never verifies** — a manager does. |
| gov.uk share-code check (§2.6) | No Edge Function. **Risk R-02:** gov.uk has no public API for the employer "view a job applicant's right to work" check, so v1 is the assisted check (ADR-0002): `packages/domain/shareCode.ts` validates the format in the wizard, step 4 files a pending `share_code_report` row carrying the code, and the office confirms the right-to-work-until date on Verify from the report (`ShareCodeCard` on `/onboarding/:id`, the Needs review row on `/compliance`; ADR-0018 makes the date mandatory). `record_document_extraction()` pre-fills that date once the extractor reads the report. If THC obtains API access or accepts a browser-automation worker, it plugs in behind `DocumentExtractor` and the same row. |
...
- GDPR Remove = Postgres function `remove_worker(staff_id)` (`20260921190118`, service role only, reached from `/staff/:id` → Remove from system): anonymises name/contacts/photo and the `applications` row, deletes the document/bank/referee rows, sets `removed_at`, unlinks the login (`profiles.staff_id`), nulls the Willo handle, releases future bookings; history rows stay. The Storage objects are not deleted in SQL — the function queues them in `storage_deletions` and the `gdpr-purge` Edge Function drains that queue (`20260922081512`), so an erasure is never lost to Storage being unreachable. Deleting the `auth.users` row itself is a GoTrue admin-API call that the office action does not yet make (docs/14 §4). Already-issued PDFs are never touched; regenerated ones read the anonymised name (§1.7).
