...
| AI document extraction (§2.6) | `DocumentExtractor` interface in `apps/staff/app/onboarding/extractor.ts` (server-side, called from the wizard's upload step) — no separate Edge Function; provider = **Gemini** (decided 17.07.2026), swappable, **not yet written**: `documentExtractor()` returns `null` until `GEMINI_API_KEY` and THC's sample letters exist (ADR-0014). The write path is built: `record_document_extraction()` (service role only) pre-fills `ai_extracted`, `ai_confidence`, dates and `needs_manual_review` against `settings.ai_confidence_threshold`; until the provider lands every upload arrives flagged for manual review. **Never verifies** — a manager does. |
