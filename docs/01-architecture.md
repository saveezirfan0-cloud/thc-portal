...
| gov.uk share-code check (§2.6) | ADR-0025. pg_cron → Edge Function `rtw-check` (a relay holding `OFFICE_BASE_URL` + `RTW_JOB_SECRET` as Supabase secrets) → Back Office `POST /api/jobs/rtw-check` on Vercel's Node runtime: Playwright + `@sparticuz/chromium` fills the Home Office form, Claude reads the result PDF, `@thc/domain` `assessRtwResult()` recommends, `record_rtw_check()` pre-fills the date and stores the report. An admin verifies or rejects through the one verify path (ADR-0018). No provider. Table `rtw_checks`; job `rtw-check` ships disabled. |
