...
- ~~`/apply` still uses its own consent tick~~ **Closed 25.09.** It hand-rolled
  ...
  makes the error *audible*: the component took an `error` prop, rendered the
  message in a sibling span, and told assistive technology nothing was wrong.
  Three hand-rolled copies remain (the HMRC declaration, the contract signature,
  the office role picker); each is a place the next D1 can hide.
