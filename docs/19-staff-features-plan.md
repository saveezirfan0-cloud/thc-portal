...
1. **The client sees none of this.** None of the seven new tables gets a client policy and no `client_*` view reads from them (ADR-0004/0026). `001_rls_guard` keeps its empty client-policy set; pgTAP `700` adds a `pg_depend` check that no `client_*` view depends on a new table.
...
**pgTAP:** 700 matrix · 701 vectors through `unavailability_range` · 702 GDPR deletes rows · 705 (B) worker RPCs — refusals `in_past`, `too_far` (> 365 d), `too_long`, `bad_window`, `too_many` (> 26 repeats or > 200 future rows); cannot touch another worker's rows; leavers/removed refused · 706 (A) `invite_worker(…,'auto'|'escalation')` refuses `unavailable`, `'manual'` still invites, `accept_invite`/`apply_to_shift` by an unavailable worker succeed, every earlier `invite_worker` refusal re-asserted in one file.

**RPCs.** B: `my_unavailability(p_from, p_to)`, `add_my_unavailability(p_from_date, p_to_date, p_from_time, p_to_time, p_repeat_weeks int default 0) → {ok, ids[], conflicts[{bookingId, event, startsAt}]}` (conflicts = worker's confirmed bookings, for the warning), `remove_my_unavailability(p_id, p_whole_series default false)`. A: `auto_assign_unavailable(p_shift) → (staff_id, starts_at, ends_at)` (admin/service); `invite_worker` restated from `20260928110200` (rebased onto main's `20260930110100` at the renumbering) + `p_source in ('auto','escalation') and staff_unavailable(...)` → `{invited:false, reason:'unavailable'}`.
...
**Owners/files.** `scheduling` (ADR, engine, board) with `staff-pwa` and `directory`. Phase 0: `packages/domain/src/{availability.ts, autoAssign.ts, index.ts, availability.vectors.json, __tests__/*}`, `packages/domain/scripts/gen-vectors-sql.mjs`, `supabase/tests/_shared/availability_vectors.psql`. A: `supabase/migrations/20260930201000_…`, `supabase/functions/auto-staffing/index.ts`, `apps/office/app/events/[id]/{board-data.ts, board-model.ts, _components/RoleBoard.tsx, _components/PotentialPool.tsx, __tests__/*}`. B: `apps/staff/app/profile/availability/**`, `ProfileHub.tsx`. C: `apps/office/app/staff/[id]/{Availability.tsx, ProfileScreen.tsx, data.ts, types.ts}`. **THC:** Q9, Q10.
...
**pgTAP:** 700 matrix + no `client_*` dependency · 702 GDPR deletes · 710 (B) save → read → clear, bad phone refused, no cross-worker access, `inactive` reads but cannot write, `removed` refused · 711 (C) office save writes `audit_log`, non-admin refused.
...
**Domain (Phase 0)** `emergencyContact.ts`: `validateEmergencyContact()` reusing `isPhone` from `onboarding.ts` (grep first; `toE164` lives in `apps/staff/app/apply/form.ts`). `emergencyContact.vectors.json` phone cases, also run by 700 against the CHECK.
...
**Owners/files.** `directory` with `staff-pwa`. B: `20260930202000_…`, `apps/staff/app/profile/details/{DetailsForm.tsx, page.tsx}`, `apps/staff/app/profile/{actions.ts, data.ts, types.ts}`, `ProfileHub.tsx`. C: `20260930203000_…`, `apps/office/app/staff/[id]/{Overview.tsx, EmergencyContactCard.tsx, actions.ts, data.ts, types.ts}`. **THC:** Q11, Q12.
...
**pgTAP:** 700, 701 (+ `490`/booking vectors with `handed_over`) · 720 offer/withdraw (refusals `too_late` at exactly 72 h, `auto_assign_off`, `already_offered`, `not_confirmed`; lapse trigger per foreign cause) · 721 take with **every gate in one file**, confirmed count unchanged (refusals `own_offer`, `taken` race, `offer_expired`, each gate by name, `not_yet`; original → `cancelled/handed_over/self_cancelled`; offerer then gated `self_cancelled`) · 722 cover (request inside 72 h, OF5, open to pool, decline + OF6, Withdraw lapses) · 723 `lapse_shift_offers` + `notify_offer_candidates` (additive, wave 1 first, never after expiry, unavailable skipped) · 724 every OF payload's keys equal its template placeholders (the `592` pattern).

**RPCs (A, `20260930201100_shift_offers.sql`).** Worker: `offer_shift(p_booking)`, `withdraw_shift_offer(p_offer)`, `request_cover(p_booking, p_note)`, `take_offered_shift(p_offer)`, `staff_open_offers()` (role, event, venue, UK window, base rate, dress code, km; no offerer). Admin: `office_open_offer_to_pool(p_offer)`, `office_decline_cover(p_offer, p_note)`. Service: `lapse_shift_offers(p_now)`, `offer_candidates(p_offer)`, `notify_offer_candidates(p_offer, p_staff uuid[])`, `offer_wave1_exhausted(p_offer)`. Trigger: `bookings_offer_lapse`. `auto-staffing` hourly gains: lapse → open pool offers → candidates → `selectOfferRecipients` → notify.
...
| OF5 | email · admin@ | `Cover requested — {event} · {role} · {date}` | worker, Employee ID, event, client, venue, role, UK window, note, `{confirmed} of {headcount} (+{buffer})`, auto-assign on/off, "They are still booked until you act." | immediately · `OF5:booking:<booking id>` (one per booking; review fix `20260930205000`) |
...
**Owner/files.** `scheduling`. A: `20260930201100_…`, `supabase/functions/auto-staffing/index.ts`, `apps/staff/app/{actions.ts, shifts/**, radar/**}`, `apps/office/app/events/[id]/**` + tests. **THC:** Q15–Q18.
...
**pgTAP:** 700 · 702 GDPR revokes the removed worker's code; referral rows kept, referrer reads "Deleted account #id" · 730 (B) minted once and stable, compliant only, summary is a count · 731 (D) valid code records a row for a `candidate_created` application only — a `returning_applicant` match records nothing (security review, `20260930205300`); bad/revoked/self code records nothing and **the application still succeeds**; throttles untouched; anon path records nothing; response identical with or without a code.
...
**Owners/files.** `onboarding` with `staff-pwa`, `directory`. B: `apps/staff/app/profile/refer/**`, `ProfileHub.tsx`. D: `20260930204000_…`, `apps/staff/app/apply/**`, `apps/staff/app/privacy/page.tsx` (one sentence), `apps/office/app/onboarding/{CandidateScreen.tsx, OnboardingBoard.tsx, data.ts, view-model.ts, __tests__/*}`. C: `apps/office/app/staff/[id]/{Overview.tsx, data.ts}`. **THC:** Q19, Q20.
...
Q9–Q21 are carried into `docs/17-inputs-from-thc.md` as items 28–40 (v1.2, 26.09; generated by `packages/pdf/scripts/thc-inputs.mjs`).
...
1. Final `gen:types` and removal of the local RPC casts. 2. `checkin`: emergency contact on the `/checkin` worker detail. 3. `scheduling`: direct offers/swaps if Q17 = yes. 4. `notifications`: RF1 if Q20 = yes. 5. ~~`reports`: Q9–Q21 into `thc-inputs.mjs`, regenerate docs/17.~~ Done 26.09 (docs/17 v1.2). 6. `security`: pass over the ~25 new definer functions.
...
**Critical references:** `packages/domain/src/state.ts`; `supabase/migrations/20260928110400_accept_gate_block_withdraws_all_and_allocation_default.sql` (the `accept_invite` gates every take path mirrors — since main's `20260930110000` the overlap counts `worked` too); latest `invite_worker` in `20260930110100_reopened_offers_and_switches_at_insert.sql` (it was `20260928110200` when this plan was written); `packages/notifications/src/templates.ts`; `supabase/tests/001_rls_guard.sql`; `apps/staff/app/profile/_components/ProfileHub.tsx`; `apps/office/app/staff/[id]/ProfileScreen.tsx`.
