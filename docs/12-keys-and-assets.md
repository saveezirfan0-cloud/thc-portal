...
| `ANTHROPIC_API_KEY` (+ optional `ANTHROPIC_MODEL`, `ANTHROPIC_EFFORT`, `DOCUMENT_EXTRACTOR`) | https://platform.claude.com/settings/keys — an API key on THC's Anthropic organisation. **Vercel, Staff App (`thc-portal-staff`) only, server-side — never `NEXT_PUBLIC_`, never Supabase.** See "Reading dates off documents" below | Switching on §2.6 document extraction (ADR-0031) |
| ~~`GEMINI_API_KEY`~~ | **Replaced** by `ANTHROPIC_API_KEY` (ADR-0031, awaiting THC's confirmation). Do not create one; no code reads it | — |
...
### Reading dates off documents (§2.6, ADR-0031)
...
awaiting THC's confirmation** (ADR-0031). It runs inside the Staff App's server actions
...
sub-processor list must name Anthropic in place of Google (ADR-0031).
