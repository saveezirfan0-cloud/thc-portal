...
| `NEXT_PUBLIC_OFFICE_URL`, `NEXT_PUBLIC_CLIENT_URL` | Each app's own public origin, e.g. `https://office.thehospitalitycompany.co.uk`. **Vercel, on the Back Office and the Client Portal respectively.** The Forgot password (A1) email links back to `<origin>/auth/callback?next=/reset`; without it Vercel's per-deployment `VERCEL_URL` is used, and outside Vercel in production the reset is refused. Add `<origin>/auth/callback` for all three apps to Supabase Auth → URL Configuration → Redirect URLs, or Supabase sends the link to the Site URL instead | Back Office / Client Portal password reset |
