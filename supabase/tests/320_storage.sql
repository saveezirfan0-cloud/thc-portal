...
select bag_eq(
  ...
  $$ values ('photos_worker_read_own:r'::text), ('photos_worker_insert_own:a'),
            ('photos_worker_update_own:w'), ('photos_admin_read:r') $$,
  'photos carries exactly a worker read/insert/update on their own folder and an admin read — and no delete for anybody (§1.7 erasure is the storage_deletions queue)'
);
...
select skip('authenticated holds no INSERT/SELECT privilege on storage.objects, so the photos policies cannot be exercised from here. Supabase grants these from the storage service, not from this repo — if this skip is showing on a real project, the photos upload will fail with 42501 and the grant is the thing to fix.', 5);
