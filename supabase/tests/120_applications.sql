...
--
-- Covers the public application migration: `applications` RLS for admin / client / staff /
...
--   §2.12 email, and mobile + date of birth exactly, route a returning
...
select throws_ok(
  $$ select submit_application('Kid','Young','kid@rls.test','+447700900091', (current_date - interval '17 years')::date, true) $$,
  '23514', 'apply_under_18', 'the server refuses an under-18 date even though the form already did');

select throws_ok(
  $$ select submit_application('Kid','Young','kid@rls.test','+447700900091', (current_date + interval '1 day')::date, true) $$,
  '23514', 'apply_dob_invalid', 'a date in the future is refused');

select throws_ok(
  $$ select submit_application('Kid','Young','kid@rls.test','+447700900091', null::date, true) $$,
  '23514', 'apply_dob_required', 'the date of birth is required, not optional');

-- ---- consent is mandatory (§1.7) ------------------------------------
select throws_ok(
  $$ select submit_application('No','Consent','noconsent@rls.test','+447700900092', date '1995-05-05', false) $$,
  '23514', 'apply_consent_required', 'nothing is created without the GDPR tick');

select throws_ok(
  $$ select submit_application('No','Consent','noconsent@rls.test','+447700900092', date '1995-05-05', null) $$,
  '23514', 'apply_consent_required', 'an absent consent is not a given consent');

-- ---- the rest of the field rules ------------------------------------
select throws_ok(
  $$ select submit_application('A','B','not-an-email','+447700900093', date '1995-05-05', true) $$,
  '23514', 'apply_email_invalid', 'an address that cannot receive the interview link is refused');

select throws_ok(
  $$ select submit_application('A','B','a@b.co','07700900093', date '1995-05-05', true) $$,
  '23514', 'apply_phone_invalid', 'the mobile has to arrive in E.164');

select throws_ok(
  $$ select submit_application('   ','B','a@b.co','+447700900093', date '1995-05-05', true) $$,
  '23514', 'apply_name_required', 'both names are required');

-- ---- a valid application (§2.1) -------------------------------------
select lives_ok(
  $$ select submit_application('Nia','Okafor','Nia.Okafor@rls.test','+447700900090', date '1994-06-15', true) $$,
...
select is(
  ...
  (extract(year from age(current_date, date '1994-06-15'))::int)::text,
  'the §2.1 age band is derived from the date of birth, never asked separately (docs/adr/0006)');

select is(
  (select dob from applications where lower(email) = 'nia.okafor@rls.test'),
  date '1994-06-15',
  'and the date itself is stored, which is what §2.12 matches on');

select ok(
  (select s.dob = date '1994-06-15' and s.willo_candidate_id is null
     from applications a join staff s on s.id = a.staff_id
    where lower(a.email) = 'nia.okafor@rls.test'),
  'the candidate carries the date of birth from creation, and no Willo record yet (§2.4 creates it)');
...
set local role anon;
select lives_ok(
  $$ select submit_application('Staff','Alpha','STAFFA@rls.test','+447700900081', date '1995-01-01', true) $$,
  'a returning applicant matched on email submits without error');
select lives_ok(
  $$ select submit_application('Different','Person','different@rls.test','+447700900071', date '1990-03-03', true) $$,
  'a returning applicant matched on a formatted mobile submits without error');
select lives_ok(
  $$ select submit_application('Blocked','Worker','blocked-again@rls.test','+447700900073', date '1990-04-04', true) $$,
  'a blocked worker can apply again');
select lives_ok(
  $$ select submit_application('Rejected','Cand','rejected@rls.test','+447700900082', date '1990-05-05', true) $$,
  'a rejected candidate can apply again');
select lives_ok(
  $$ select submit_application('Inactive','Leaver','inactive@rls.test','+447700900083', date '1990-06-06', true) $$,
  'a leaver can apply again');
select lives_ok(
  $$ select submit_application('Nia','Okafor','nia.okafor@rls.test','+447700900090', date '1994-06-15', true) $$,
  'and so can somebody who simply submitted the form twice');
select lives_ok(
  $$ select submit_application('Deleted','Account','removed@rls.test','+447700900072', date '1990-01-01', true) $$,
...
-- ---------------------------------------------------------------------
-- The date of birth (docs/adr/0006)
--
-- Now that the form collects it, every candidate has one from creation
-- and the only row allowed to lose it is an anonymised one (§1.7).
-- ---------------------------------------------------------------------
...
select lives_ok(
  ...
  'a candidate with a date of birth moves through the pipeline freely');

select throws_ok(
  format($$ update staff set dob = null where id = %L $$, :nia_id),
  '23514', null,
  'and cannot simply have it removed while they are still a live record');

select lives_ok(
  format($$ update staff set status = 'removed', dob = null, removed_at = now() where id = %L $$, :nia_id),
  'only a GDPR removal may clear it, which is the one case §1.7 requires');
