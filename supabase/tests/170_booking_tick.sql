...
insert into shift_requirements (id, event_id, role_id, starts_at, ends_at, headcount, buffer,
  ...
  -- Two sections TOMORROW (22 Sep, Europe/London) for the §3.5 stage-2 push.
  ('51000000-0000-4000-8000-000000000009', :'evt_unpaid', :'role_id', '2026-09-22 18:00+00', '2026-09-23 00:00+00', 1,0, 30,15, 1),
  ('51000000-0000-4000-8000-000000000010', :'evt_unpaid', :'role_id', '2026-09-22 18:00+00', '2026-09-23 00:00+00', 1,0, 30,15, 1);
...
-- ---------------------------------------------------------------------
-- §3.5 stage 3 · N7 "Confirm today's shift", on the day, before the start.
--   A reminder only: nothing releases on it.
-- ---------------------------------------------------------------------
select is((select counts->>'n7' from t_first), '1',
  '§3.5 stage 3: N7 goes to the one confirmed booking whose section starts later today (21:20 London) and has no on-the-day press');
select ok(
  exists (select 1 from notification_outbox where key = 'N7:booking:b1000000-0000-4000-8000-000000000001'),
  'keyed on the booking, so the every-minute job sends it once');
select ok(
  not exists (select 1 from notification_outbox where key = 'N7:booking:b1000000-0000-4000-8000-000000000004'),
  'a section that has already started gets no N7 — that worker is a No-show, not a reminder');
...
-- (end + 4 h, RULE-02): past it the worker has gone home without checking
-- out, BG-09 has raised No check-out, and nobody is told to ask a manager
-- on site about a break.
-- ---------------------------------------------------------------------
select is((select counts->>'n13' from t_first), '3',
  'BG-10 alerts the three workers six hours in and still checked in on an unpaid-break client — two on shift, one an hour past the scheduled end');
select ok(
...
select ok(
  ...
  '§5.2b a shift running an hour past its scheduled end, still checked in, is still prompted — the breaks block does not disappear at the planned end');
select ok(
...
-- ---------------------------------------------------------------------
-- §3.5 stage 2 · N6 "Confirm tomorrow's shift by 12:00 today", the
--   morning before (from 08:00 UK) until the deadline. Run at 08:30 UK on
--   the 21st for the two sections on the 22nd: nothing else in the
--   fixture is due at that instant.
-- ---------------------------------------------------------------------
create temporary table t_morning as select booking_tick(timestamptz '2026-09-21 07:30+00') as counts;

select is((select counts->>'n6' from t_morning), '1',
  '§3.5 stage 2: N6 goes to the one confirmed booking for tomorrow that has no day-before press and a deadline still to meet');
select ok(
  exists (select 1 from notification_outbox where key = 'N6:booking:b1000000-0000-4000-8000-000000000009'),
