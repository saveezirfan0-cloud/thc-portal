...
select bag_eq(
  ...
  'A: each carries exactly one policy — admin_read, SELECT — and nothing for staff, client or anon (docs/19 §0)');
  ...
  $$ select distinct v.relname::text || ' -> ' || t.relname::text
      ...
      where d.classid = 'pg_rewrite'::regclass
        ...
        and t.relname in ('staff_unavailability', 'staff_emergency_contacts',
                          'profile_change_requests', 'shift_offers', 'shift_offer_notices',
                          'staff_referral_codes', 'application_referrals') $$,
  'B: no client_* view depends on a docs/19 table — the client sees none of it');
...
select throws_ok(
  ...
  '42501', null,
  'C admin: cannot write a row directly either — office writes are audited definer RPCs (docs/19 §0.3)');
select throws_ok(
