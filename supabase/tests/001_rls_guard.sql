...
select bag_eq(
  ...
  'RLS is enabled on all 37 tables: the 17 from 0001_init.sql, the 11 closed by 0004_rls_gaps, job_runs + job_schedules from the jobs layer, applications from the public form, cap_band_notices from the compliance job, staff_transitions from the §2.12 machine, storage_deletions from §1.7''s Storage half, and the three the §10.3 wizard added (onboarding_progress, quiz_questions, contract_versions)'
);
