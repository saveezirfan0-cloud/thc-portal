...
-- exists to serve.
--
-- 0006 finished the job. client_events_v was still an invoker view, which
...
select ok(not (coalesce((select reloptions from pg_class where relname = 'client_events_v'), '{}') @> '{security_invoker=true}'),
  'client_events_v runs with owner rights (ADR-0004, 0006), so event_windows need not be granted to the caller');
...
-- PostgREST roles while doing so is not, and it was until 0006. The guard
...
select ok(not has_table_privilege('anon', 'event_windows', 'select'),
  'anon holds no privilege on event_windows (0006): owner rights over shift_requirements must not be world-granted');
