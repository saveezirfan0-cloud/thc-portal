...
--       ever pushed (20260930150000, QA S1: the RULE-17 deadlock)
--   F · ask / withdraw / ask again: recently_requested within 24 h, and
--       OF5 keyed on the booking, so admin@ is emailed once (security #1)
--
-- The callers (20260930150000): a leaver or a removed account cannot ask.
-- =====================================================================
begin;
select plan(56);
...
select throws_ok(format($$ select request_cover(%L) $$, :'b_in'), 'P0001', 'not_editable',
  'A: a leaver cannot ask the office for cover (20260930120000''s error shape)');
reset role;
...
select is(
  ...
  'C: the audit row records that a note was written, never its text (20260930150000)');
select is(
