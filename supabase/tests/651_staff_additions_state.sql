-- =====================================================================
-- 651 · The staff additions — machines, builders and CHECKs
--   20260930100100_staff_additions_schema.sql · docs/18 · ADR-0036 … 0039
--
--   A. profile_change_requests (ADR-0038): profile_change_transitions() is
...
--   B. shift_offers (ADR-0039): shift_offer_transitions() and
...
--   C. staff_unavailability (ADR-0036): unavailability_range() and
...
-- the clock and the worker's other rows, and are the RPC's (655).
...
select ok('offer' = any (enum_range(null::booking_source)::text[]),
  'D: booking_source carries offer (20260930100000)');
...
select lives_ok(
  ...
  'D: a confirmed booking is handed over: cancelled / handed_over, self_cancelled (ADR-0039)');
