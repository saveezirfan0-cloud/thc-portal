-- =====================================================================
-- 661 · Additive rounds keep adding; ended offers can be made again
--   20260930110100_reopened_offers_and_switches_at_insert.sql (audit D33, D9)
--
--   1. D3 (fixed on main by 20260928110200, held here against the
--      restated invite_worker): a second round invites `allocation` more
--      while the first round's invitations are still unanswered, and the
--      rounds stop once headcount + buffer is CONFIRMED (open invitations
--      never count).
...
select ok(booking_transition_allowed('cancelled', 'invited') and booking_transition_allowed('closed', 'invited')
  ...
  'the three reopen edges exist (ADR-0037)');
