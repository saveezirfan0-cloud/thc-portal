-- =====================================================================
-- 651 · Resolve on a No-show, before and after the end (§3.3, §5.1, §9.5)
--   20260930100000_check_in_out_corrections.sql · ADR-0035
--
--   main's get_back(uuid, text) (20260928110200, test 597) still works
--   through the restated resolve_violation(): before the end the press is
--   the arrival, the shift pays, the worker is unlocked, and the next
--   booking_tick() has no No-show left to raise.
...
--        shift settles instead of staying Pending. get_back() sends no
--        arrival, so after the end it answers arrived_at_required.
-- =====================================================================
begin;
...
\set ev      '65100000-0000-4000-8000-0000000000e1'
\set sh_run  '65100000-0000-4000-8000-0000000000f1'
\set sh_done '65100000-0000-4000-8000-0000000000f2'
\set bk_run  '65100000-0000-4000-8000-0000000000b1'
\set bk_done '65100000-0000-4000-8000-0000000000b2'
...
-- ---------------------------------------------------------------------
-- Get back on the event board, before the end (main's get_back)
-- ---------------------------------------------------------------------
set local "request.jwt.claims" = '{"sub":"11111111-1111-1111-1111-111111111111","role":"authenticated"}';
select is(get_back(:'bk_run')->>'nowType', 'late',
  'Get back, through the restated resolve_violation, reclassifies the No-show to Late (§3.3)');
...
select throws_ok(format('select get_back(%L)', :'bk_run'), 'P0002', 'no_open_no_show',
...
-- ---------------------------------------------------------------------
set local "request.jwt.claims" = '{"sub":"11111111-1111-1111-1111-111111111111","role":"authenticated"}';
\set v_done '(select id from violations where booking_id = ''65100000-0000-4000-8000-0000000000b2'' and type = ''no_show'')'
...
select throws_ok(format('select get_back(%L)', :'bk_done'),
  'P0001', 'arrived_at_required',
  'D17: so the board''s Get back, which sends no arrival, is refused after the end too');
