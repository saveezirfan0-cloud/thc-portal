-- =====================================================================
-- 650 · check-out corrections (§5.1, §5.2, §5.2b, §9.5, RULE-01/02/14)
--   20260930100000_check_in_out_corrections.sql · ADR-0035
--
...
\set ev     '65000000-0000-4000-8000-0000000000e1'
\set sh_d5  '65000000-0000-4000-8000-0000000000f1'
\set sh_gr  '65000000-0000-4000-8000-0000000000f2'
\set sh_d15 '65000000-0000-4000-8000-0000000000f3'
\set sh_d16 '65000000-0000-4000-8000-0000000000f4'
\set bk_d5  '65000000-0000-4000-8000-0000000000b1'
\set bk_gr  '65000000-0000-4000-8000-0000000000b2'
\set bk_d15 '65000000-0000-4000-8000-0000000000b3'
\set bk_blk '65000000-0000-4000-8000-0000000000b4'
\set bk_run '65000000-0000-4000-8000-0000000000b5'
\set st_blk '65000000-0000-4000-8000-0000000000d1'
\set us_blk '65000000-0000-4000-8000-0000000000a1'

insert into auth.users (id, email) values (:'us_blk', 'blk650@rls.test');
...
insert into staff (id, user_id, employee_id, first_name, last_name, email, phone, dob, status) values
  (:'st_blk', :'us_blk', 96501, 'Blocked', 'Later', 'blk650@rls.test', '+447700965001', date '1993-03-03', 'compliant');
...
select is(check_out(:'bk_gr', 51.5000, -0.1000)->>'leftEarly', 'false',
  'D5 / ADR-0035: ten minutes before the end is inside the 15-minute grace');
...
reset role;
select ok((select stale_fix_review from violations where booking_id = :'bk_d15' and type = 'no_checkout'),
  'D15: the row is marked as the stale-fix review');
-- The worker pressed Check out: the review never counts against the
-- show-rate (main's staff_show_rate, restated). Unmark it and it would.
create temporary table sr650 on commit drop as select staff_show_rate(:'staffb') as r;
update violations set stale_fix_review = false where booking_id = :'bk_d15' and type = 'no_checkout';
select ok(staff_show_rate(:'staffb') < (select r from sr650),
  'D15: the same row as an ordinary No check-out would lower the show-rate …');
update violations set stale_fix_review = true where booking_id = :'bk_d15' and type = 'no_checkout';
select is(staff_show_rate(:'staffb'), (select r from sr650),
  'D15: … so the stale-fix review is left out of it (ADR-0035)');
...
set local "request.jwt.claims" = '{"sub":"65000000-0000-4000-8000-0000000000a1","role":"authenticated"}';
