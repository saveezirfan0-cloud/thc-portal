-- =====================================================================
-- Migration 20260930140000 · /apply?ref= records who referred whom
--                            (ADR-0040; docs/18 §5; §2.1, §2.3, §1.7)
--
...
--
--   2 · submit_application_as_caller — RESTATED (docs/18 §0.6, the only
...
--
-- What does NOT change (docs/10 §3b — pgTAP 681 asserts every gate
-- together):
...
-- (20260930100100), and nothing here reaches pay, reports or PDFs (Q19).
-- =====================================================================
...
begin
  ...
  begin
    ...
    -- A revoked code (GDPR removal, 20260930100100) records nothing; so
  ...
  exception when others then
    -- Never raises (ADR-0040 point 4). The block is its own
...
comment on function public.record_application_referral(text, text) is
  'ADR-0040: records who referred an application that arrived through /apply?ref=. Finds the application submit_application() just wrote for this email (same normalisation), skips a malformed, unknown, revoked or own code, inserts on conflict do nothing, and never raises. Owner-only: called by submit_application_as_caller(), by no API role.';
...
create or replace function public.submit_application_as_caller(
  ...
  p_consent     boolean,
  p_caller_hash text,
  -- ADR-0040: /apply?ref=. Recorded after the application is written,
...
begin
  ...
  -- ADR-0040: the one new clause. Only reached once the application is
