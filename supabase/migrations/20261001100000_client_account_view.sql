...
-- addresses its documents go to (ADR-0051, ADR-0004)
--
...
comment on view client_account_v is
  'The signed-in customer''s own company name and the addresses THC emails its allocation sheet and signed timesheet to (§9.7, §11.4), for the Client Portal''s Your account page (ADR-0051). Owner rights + current_client_id() + client_portal_visible() per ADR-0004: the client role holds no policy on clients and must not be given one. One row for a client user, none for anybody else. Read-only; no terms, no phone, no money, nothing about workers.';
