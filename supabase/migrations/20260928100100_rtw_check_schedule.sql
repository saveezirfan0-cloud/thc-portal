...
--
-- Two columns let a registry row name where its base URL comes from and
-- its bearer secret. Every existing row keeps edge_base_url /
-- service_role_key, so their pg_cron commands post to the same URL with
-- the same key. The office route is called with its OWN secret (vault
-- rtw_job_secret, env RTW_JOB_SECRET on the Vercel project), never the
-- service key, at a base held in the VAULT (secret office_base_url), never
-- in settings, which an admin session can write (security review, 26.09).
--
...
alter table job_schedules
  add column if not exists base_url_source text not null default 'edge_base_url',
  add column if not exists secret_name     text not null default 'service_role_key';

alter table job_schedules drop constraint if exists job_schedules_base_url_source;
alter table job_schedules add constraint job_schedules_base_url_source
  check (base_url_source in ('edge_base_url', 'office_base_url'));
-- Only the two bearers there are, each with its own reader.
alter table job_schedules drop constraint if exists job_schedules_secret_name;
alter table job_schedules add constraint job_schedules_secret_name
  check (secret_name in ('service_role_key', 'rtw_job_secret'));
...
alter table job_schedules add constraint job_schedules_service_key_to_edge_only
  check (secret_name <> 'service_role_key' or base_url_source = 'edge_base_url');

comment on column job_schedules.base_url_source is
  'Where the base URL edge_path is appended to comes from: edge_base_url — settings.edge_base_url through edge_base_url(), a Supabase Functions base only (20260927160300); office_base_url — the VAULT secret office_base_url through office_base_url(), a Back Office origin (ADR-0025).';
...
insert into job_schedules (job, cron_expression, edge_path, enabled, note, base_url_source, secret_name) values
  ('rtw-check', '*/10 * * * *', 'api/jobs/rtw-check', false,
   '§2.6 automated right-to-work check (ADR-0025): a Back Office Node route (apps/office/app/api/jobs/rtw-check), not an Edge Function — the gov.uk fallback needs Chromium. Every 10 min; share codes filed in between also nudge it. Enable once settings.rtw_check.enabled, the vault secrets office_base_url and rtw_job_secret, and the office''s RTW_* env are set (OWNER-TODO §8).',
...
-- through office_base_url() (20260928100000, from the vault), which re-checks the same
...
comment on function public.install_job_schedules() is
  'Applies job_schedules to pg_cron. Run once per deploy; needs settings.edge_base_url (a Supabase Functions base — edge_base_url() refuses anything else) and vault secret service_role_key, and for an office_base_url row (rtw-check) the vault secrets office_base_url (office_base_url() refuses anything but a Back Office origin) and rtw_job_secret — without them that row is skipped with a notice.';
