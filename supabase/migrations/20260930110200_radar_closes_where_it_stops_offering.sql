-- =====================================================================
-- Migration 20260930110200 · Radar stops offering, and applications close
...
-- (booking_reopenable_by: slot_taken is `anyone`). ADR-0037.
--
-- Radar also offers a section again to a worker whose earlier row on it
-- ended in a way the worker may reopen (20260930110100) — not only a
-- `closed` one, as 20260928110500 had it; a self-cancel is still gated
-- `self_cancelled` and never offered.
--
-- Bases:
--   • close_filled_role_applications(), accept_application() —
--     20260925100000, the only versions.
--   • staff_open_shifts() — 20260928110500, ALL 26 columns kept in their
--     order (the Radar map reads the venue and home pins) and its grants
--     restated. Only the section join, the applied_at read and the WHERE
--     change; the return type does not, so this is create or replace.
-- =====================================================================
...
begin
  ...
  -- apply_to_shift): no seat left against headcount (20260930110200).
...
comment on function public.close_filled_role_applications(uuid) is
  '§8 N10c: once a role section has no seat left (confirmed-or-worked >= headcount — the point Radar stops offering it, 20260930110200), every still-pending Radar application on it closes (closed / slot_taken) and its worker gets N10c. Called by accept_application and accept_invite after they confirm.';
...
begin
  ...
  -- One threshold with Radar and N10c (20260930110200): an application
  -- is for a seat, and there is none left at confirmed >= headcount.
  -- shift_fill counts confirmed-or-worked (20260930110000).
...
comment on function public.accept_application(uuid) is
  'The office takes a Radar application forward (§3.3, §10.4): applied → confirmed, admin only. Refusals: event_cancelled / not_applied / event_ended / full (confirmed-or-worked >= headcount, the Radar/N10c threshold, 20260930110200) / not_bookable / the auto-assign hard gates by name. Queues N10, withdraws the worker''s overlapping invitations and, once the role has no seat left, closes the other applications with N10c.';

-- ---------------------------------------------------------------------
-- 3 · staff_open_shifts — 20260928110500, all 26 columns, but the
--     worker's own application stays until it resolves
-- ---------------------------------------------------------------------
...
set search_path = public, extensions as $$
  ...
  select
    ...
    -- A pending application can outlive the worker's candidate row (the
    -- wave or a gate moved underneath it), so "not known" reads false.
  ...
  where
    ...
    or (
      ...
      -- shifts. An ended row the worker may reopen by applying — closed
      -- (declined, withdrawn, a slot that went to somebody else) or, since
      -- 20260930110100, cancelled by the office or the 12:05 cutoff — is
      -- offered again; one they may not (never: a self-cancel, an event
      -- cancellation, a GDPR removal) is not.
...
comment on function public.staff_open_shifts(uuid) is
  'Radar and the Shifts tab''s Open shifts (§10.4): open sections (confirmed-or-worked < headcount) for the worker''s own roles, qualified clients first (RULE-17), closest first, with the RULE-20 cap state per row and the venue and home pins for the detail''s map (ADR-0005). The worker''s own pending application stays listed until it resolves or the section ends; an ended row the worker may reopen is offered again (20260930110200).';
