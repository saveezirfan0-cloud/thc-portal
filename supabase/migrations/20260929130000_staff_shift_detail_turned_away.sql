-- =====================================================================
-- staff_shift_detail() carries the turn-away attempt (§3.2, RULE-15)
--
-- Audit D19: a worker turned away under the strict buffer policy was
-- offered Check in again on reload, because the shift screen had no way to
-- know it had happened beyond the one-off message the press returned. The
-- booking is `turned_away` (20260924120000), which the screen now reads as
-- its own full-screen "Thanks for coming" state; what it could not read is
-- WHEN the attempt was logged, and RULE-15 prices exactly that: on time →
-- a flat 4 hours, late → nothing. The §3.2 copy's paid-4-hours sentence is
-- shown "only where the attempt was inside the grace window", so the screen
-- needs the stamp to say it truthfully after a reload.
--
-- The worker's own row only, like every other column here. `check_logs`
-- stays closed to the staff role (ADR-0004); this definer function is the
-- one path, and it names the column rather than widening that policy.
--
-- Restated in full from 20260927110000 (the latest definition). The result
-- gains a column, which `create or replace` cannot do, so it is dropped
-- and recreated; nothing depends on it but the Staff App.
-- =====================================================================
...
comment on function public.staff_shift_detail(uuid) is
  'The shift screen (§10.4, §5.1) for ONE of the caller''s own bookings: role window, base pay rate, venue and its centre, the accepted check log, breaks, the three static-screen inputs and the RULE-15 turn-away stamp. No charge rate, margin, PO or client. Another worker''s id returns no row.';
