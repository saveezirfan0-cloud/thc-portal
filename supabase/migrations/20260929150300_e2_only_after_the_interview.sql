...
-- (audit D44; §2.7, §8 E2 / E2b; ADR-0017, amended by ADR-0037)
--
...
begin
  ...
  -- rejected after the interview stage, get E2b (ADR-0017, ADR-0037).
  v_template := case
...
comment on function public.onboarding_do_reject(uuid, text, text, uuid, timestamptz) is
  'The one rejection path for a candidate (office Reject, Willo, the quiz): status rejected with cause, reason and who; E2 only when the interview has been done (Interview completed, or a Willo response on file), E2b otherwise (ADR-0017, ADR-0037). The reason never goes to the candidate.';
