-- =====================================================================
-- Migration 20260930130400 · the NI number beside its evidence, the
-- office's two uploads, and right-to-work changes in the audit export
-- (audit D43, D47, D31, AC7; ADR-0040;
...
-- replaced (the earlier one is evidence too, and would be orphaned), and
-- while the automated check owns the document (rtw_check_manual_allowed()
-- false) the office cannot attach one: rtw_check_record() writes
-- gov_report_path itself.
--
...
--   compliance_review_queue_v    20260928110900 (main's latest)
...
begin
  ...
  -- The manual path only: while the automated check owns this document
  -- (on, and not in needs_review or stuck), rtw_check_record() writes
  -- gov_report_path itself and would overwrite what the office attached.
  -- Same gate as the manual Verify (rtw_check_manual_allowed, ADR-0025).
  if not coalesce(rtw_check_manual_allowed(d.id), false) then
    return jsonb_build_object('ok', false, 'reason', 'automated_check_owns_report');
  end if;
...
comment on function public.compliance_attach_rtw_report(uuid, text) is
  'D31: the office attaches the gov.uk right-to-work report (PDF, JPG or PNG under <staff_id>/share-code-report/) to a share code document when it verified by hand (ADR-0018) — the automated check (ADR-0025) stores its own, so this is refused while rtw_check_manual_allowed() is false. Never replaces a report already attached. Admin only; audited as rtw.report_attached.';

-- ---------------------------------------------------------------------
-- 5 · Needs review, with the NI number and the conditions the reviewer sets
--
-- Restated from main's latest, 20260928110900 (every column, predicate and
-- comment carried, manual_review_reason included). Added, APPENDED after
-- manual_review_reason in all five arms so every existing column keeps
-- its place:
...
--     now that it is (D43). manual_review_reason is null on it.
-- ---------------------------------------------------------------------
...
select
  ...
  -- 20260930130400 (D43, D32, D36, D31), appended after
  -- manual_review_reason so every existing column keeps its place.
...
where k.status = 'needs_review'
  ...
  and s.removed_at is null
union all
-- 20260930130400 (D43): NI evidence verified before the number existed.
...
comment on view compliance_review_queue_v is
  '§4.1 Needs review: every pending document and every pending Yes declaration on a live profile (Rejected and Removed drop out); kind ''rtw_date'', every live non-UK worker whose latest verified share code report carries no right-to-work date while their own date is NULL (20260927160000, ADR-0018); and, with the automated right-to-work check on (ADR-0025), not a share code whose check is still running (unless stuck), plus kind ''rtw_check'', a no-right-to-work check whose document is no longer pending; and kind ''ni_check'', NI evidence verified before the NI number was entered, once it has been (D43, 20260930130400). Each document row carries its latest check; review_reason says why a non-pending row is here; manual_review_reason says why the extractor flagged a document beyond its confidence — "letter expired" for a term letter whose dates are all past, which Verify refuses (§4.2, 20260928110300, 20260928110900). NI evidence rows carry the full NI number for the reviewer; every row carries the course level and visa hours limit on file and whether a gov.uk report is attached (20260930130400). security_invoker.';
