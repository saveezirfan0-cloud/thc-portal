-- =====================================================================
-- Integration round after ADR-0055 … ADR-0058: the database now enforces
...
--         through the API (ADR-0057 decision 1).
...
comment on function public.current_app_role() is
  'The caller''s app role for RLS and every admin RPC. NULL — no access — when there is no session, the login is switched off (banned_until in the future), or a Back Office login with a verified two-step factor has not passed the code step (aal2). 20261001200500.';
...
begin
  ...
  -- ADR-0056: sending a set-up link is Users & access, which is the owner's.
...
-- create or replace keeps the grants 20261001200100 / 120000 set; restated
