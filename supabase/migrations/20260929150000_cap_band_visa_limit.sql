...
-- limit, so the 48-hour opt-out cannot lift it. 20260929150100 applies it
...
comment on type cap_band is
  'RULE-20 band (§4.4-4.5 + the University Completion Letter requirement). visa_limit (20260929150000) is a weekly hours limit written on a work or dependant visa, captured at the right-to-work check. Mirrors CapBand in packages/domain/src/cap.ts one label for one label; held to packages/domain/src/cap.vectors.json on both sides.';
