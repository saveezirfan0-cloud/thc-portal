-- =====================================================================
-- Migration 20260930130000 · the office side of the staff additions
--   docs/18-staff-features-plan.md Phase 1, Agent C (directory)
--   ADR-0036 (availability, read-only tab), ADR-0037 (emergency contact),
--   ADR-0038 (request a change), ADR-0040 (referrals)
--   — additions to Scope v1.6, status proposed — awaiting THC
--
-- Phase 0 (20260930100100) laid the tables with one admin_read policy each
...
--
-- The rules this file keeps (docs/18 §0):
...
--   • Nothing here restates a frozen function (docs/18 §0.6) or touches
--     another Phase-1 agent's objects.
--
-- pgTAP: 661 (emergency contact + the /staff/:id reads), 666 (decide).
-- Forward-only.
-- =====================================================================

-- ---------------------------------------------------------------------
-- §1 · Emergency contact (ADR-0037)
-- ---------------------------------------------------------------------
...
begin
  ...
  if v_phone !~ '^\+[1-9][0-9]{6,14}$' then
    raise exception 'bad_phone' using errcode = '22023',
      hint = 'ADR-0037: E.164 with the country code, the /apply rule.';
  end if;
...
comment on function public.office_save_emergency_contact(uuid, text, text, text) is
  'ADR-0037: the office saves or corrects a worker''s emergency contact (/staff/:id Overview → Edit). Admin only; refuses a removed worker; name 1–100, relationship 1–40, phone E.164 after separators are stripped (bad_name / bad_relationship / bad_phone, 22023). Writes audit_log emergency_contact.office_save with the changed field names — never the values. No notification.';
...
comment on function public.office_clear_emergency_contact(uuid) is
  'ADR-0037: the office clears a worker''s emergency contact (/staff/:id Overview → Clear). Admin only; audited (emergency_contact.office_clear) when there was a row to clear; clearing nothing is a no-op, not an error.';

-- ---------------------------------------------------------------------
-- §2 · Decide a change request (ADR-0038)
--
...
begin
  ...
  if r.status <> 'pending' then
    raise exception 'already_decided' using errcode = 'P0001',
      hint = 'ADR-0038: approved, rejected and withdrawn are terminal. "Request again" is a new row.';
  end if;
...
comment on function public.office_profile_change_requests(uuid, boolean, int) is
  'ADR-0038: the /staff/requests queue — pending oldest first, or decided newest first — optionally for one worker (the /staff/:id banner). Admin only. Names the decider (profiles.full_name, which an admin cannot read directly). A removed worker reads "Deleted account #id" with no photo or evidence path.';
...
comment on function public.office_emergency_contact(uuid) is
  'ADR-0037: the worker''s emergency contact for the /staff/:id Overview card, with who saved it last (worker / office + name). Admin only. Null when none — the card says "Not provided". Never read by a PDF, a client_* view or the Client Portal.';
...
comment on function public.office_staff_unavailability(uuid, timestamptz, timestamptz) is
  'ADR-0036: the /staff/:id Availability tab — a worker''s entries overlapping [p_from, p_to) (default the next 8 weeks), with the repeat series size and last start, and any confirmed booking whose role-section window overlaps (RULE-18). Admin only; read-only — the worker edits in the app.';

-- The Referrals card and the "Referred by" line (ADR-0040). A removed
...
comment on function public.office_staff_referrals(uuid) is
  'ADR-0040: the /staff/:id Referrals card — the worker''s code, who referred them (latest), and everyone who applied with their code, with status. Admin only; a removed person reads "Deleted account #id". No money, no reward (Q19).';
