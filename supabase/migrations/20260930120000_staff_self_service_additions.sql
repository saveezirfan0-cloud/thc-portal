-- =====================================================================
-- Migration 20260930120000 · the worker's availability calendar
--   docs/18-staff-features-plan.md §1 (Phase 1, Agent B · staff-pwa)
--   ADR-0036 (proposed — awaiting THC) · Q9, Q10
--
...
-- Phase 0 (20260930100100); this file references nothing newer.
--
...
-- holds no policy on staff_unavailability (docs/18 §0.2), so these are the
...
-- the shift." (ADR-0036 §1); the booking, and any open invitation, are
...
comment on function public.my_unavailability(timestamptz, timestamptz) is
  'ADR-0036: the calling worker''s own availability entries still running at p_from (default now) and starting before p_to. Takes no staff id. Refuses a leaver (not_editable) and a removed worker (account_closed).';
...
comment on function public.remove_my_unavailability(uuid, boolean) is
  'ADR-0036: deletes one of the calling worker''s availability entries, or (p_whole_series) it and every copy of its series not yet over. not_found for an id that is not theirs.';

-- ---------------------------------------------------------------------
-- Grants (docs/18 §0.2; pgTAP 190 2e/2f).
-- ---------------------------------------------------------------------
