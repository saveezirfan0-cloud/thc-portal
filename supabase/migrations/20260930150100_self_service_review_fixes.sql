-- =====================================================================
-- Migration 20260930150100 · review fixes for the worker's self-service
--   additions (docs/18 §1 and §3, Phase 1 Agent B · staff-pwa)
--
...
-- exactly as before.
--
--   1 · request_profile_change   (from 20260930120200)
...
--
--   2 · add_my_unavailability    (from 20260930120000)
--   3 · remove_my_unavailability (from 20260930120000)
...
--
-- Forward-only. pgTAP: 655 (G), 665 (G).
-- =====================================================================

-- ---------------------------------------------------------------------
-- 1 · request_profile_change — 20260930120200 + the 24-hour ceiling.
-- ---------------------------------------------------------------------
...
begin
  ...
  if exists (select 1 from profile_change_requests r
    ...
    raise exception 'already_pending' using errcode = 'P0001',
      hint = 'ADR-0038: one pending request per kind. Withdraw it to ask again.';
  end if;
  -- 20260930150100: at most three of a kind in any 24 hours, any status.
  ...
  if p_kind = 'name' then
    ...
    if v_evid is null then
      raise exception 'evidence_required' using errcode = 'P0001',
        hint = 'ADR-0038 / Q13: a name change needs evidence.';
    end if;
...
comment on function public.request_profile_change(text, text, text, text, text, text) is
  'ADR-0038: the calling worker asks the office to change their locked name (first/last + evidence in documents/<id>/change-requests/) or photo (a fresh object in photos/<id>/). One pending per kind, and at most three of a kind created in any 24 hours whatever their status (too_many_requests — each queued an RC1; 20260930150100); the uploaded object must exist; a name equal to the current one is refused. Queues RC1 to admin@ in the same transaction. Never writes staff.';

-- ---------------------------------------------------------------------
-- 2 · add_my_unavailability — 20260930120000, editable only when
...
comment on function public.add_my_unavailability(date, date, time, time, int) is
  'ADR-0036: saves one availability entry for the calling worker (UK dates/times; all day when both times are null) plus p_repeat_weeks weekly copies sharing a series_id. {ok:false, reason} for bad_window › in_past › too_far › too_long › too_many (validateUnavailability() in packages/domain). Returns the worker''s confirmed bookings the entries overlap (by role section, RULE-18) as conflicts — never cancels them. Only for a worker the Staff App would show the screen to (appLock() none: compliant, no expired document, no declaration under review); every block, a manual hold included, raises not_editable (20260930150100).';

-- ---------------------------------------------------------------------
-- 3 · remove_my_unavailability — 20260930120000, the same gate.
-- ---------------------------------------------------------------------
...
begin
  ...
  -- The same gate as add_my_unavailability (20260930150100): only when
  -- appLock() would be 'none'.
  if v_status is distinct from 'compliant'
     or exists (select 1
...
comment on function public.remove_my_unavailability(uuid, boolean) is
  'ADR-0036: deletes one of the calling worker''s availability entries, or (p_whole_series) it and every copy of its series not yet over. not_found for an id that is not theirs. Only for a worker the Staff App would show the screen to (appLock() none); every block, a manual hold included, raises not_editable (20260930150100).';

-- ---------------------------------------------------------------------
-- Grants — as 20260930120000 and 20260930120200 set them.
-- ---------------------------------------------------------------------
