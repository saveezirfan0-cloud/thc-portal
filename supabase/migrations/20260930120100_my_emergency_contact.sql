-- =====================================================================
-- Migration 20260930120100 · the worker's own emergency contact
--   docs/18-staff-features-plan.md §2 (Phase 1, Agent B · staff-pwa)
--   ADR-0037 (proposed — awaiting THC) · Q11, Q12
--
--   my_emergency_contact()                         read
--   save_my_emergency_contact(name, relationship, phone)
--   clear_my_emergency_contact()
--
-- The table is Phase 0 (20260930100100): admin_read only, no staff and no
...
-- the table (pgTAP 650).
--
-- Who may do what (ADR-0037 "Consequences"):
...
comment on function public.my_emergency_contact() is
  'ADR-0037: the calling worker''s own emergency contact ({name, relationship, phone, updatedAt}) or null. A leaver may read it; a removed worker is refused (account_closed). Takes no staff id; never returns updated_by.';
...
comment on function public.save_my_emergency_contact(text, text, text) is
  'ADR-0037: saves the calling worker''s emergency contact (name 1–100, relationship 1–40, phone E.164 after dropping separators). Raises name_required, name_too_long, relationship_required, relationship_too_long, bad_phone; not_editable for a leaver; account_closed for a removed worker. No notification.';
...
comment on function public.clear_my_emergency_contact() is
  'ADR-0037: removes the calling worker''s emergency contact. not_editable for a leaver; account_closed for a removed worker.';
