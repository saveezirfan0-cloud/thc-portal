-- =====================================================================
-- Migration 20260929140200 · worker write paths close behind their RPCs,
...
comment on policy staff_self_push on push_subscriptions is
  'The worker reads their own device subscriptions. Writes go through save_push_subscription() / forget_push_subscription() only (20260929140200).';
...
comment on function public.staff_save_bank(text, text, text) is
  '§2.10/§10.1: the ONLY worker write path to bank_details (the direct self insert/update policies were dropped in 20260927120100). Validates sort code and account number and queues E5 to payroll in the same transaction, one E5 per save (microsecond key, 20260929140200).';
...
comment on function public.submit_application(text, text, text, text, date, boolean) is
  '§2.1 /apply: validation, the 18+ gate, the §2.12 duplicate check and the write. Service role only since 20260929140200: the Staff App calls it through submit_application_as_caller(), which adds the per-caller limit (ADR-0024), and anon no longer reaches it past that limit.';
