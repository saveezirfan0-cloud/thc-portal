-- =====================================================================
-- Migration 20260930100100 · the staff additions — schema only
--   docs/18-staff-features-plan.md, Phase 0-A; ADR-0036 … ADR-0040
...
-- Phase-1 migrations (Agent A 20260930110000/110100, B 120000, C 130000,
-- D 140000), which reference only what is created here and what already
-- exists.
--
--   §1 staff_unavailability      ADR-0036 availability calendar — a hard gate
--                                for automated invitations only
--   §2 staff_emergency_contacts  ADR-0037 office-only worker data, never on a
--                                client document
--   §3 profile_change_requests   ADR-0038 request a change of name or photo
--   §4 shift_offers,             ADR-0039 offer up a shift: released only
--      shift_offer_notices       when a confirmed replacement takes it
--   §5 staff_referral_codes,     ADR-0040 refer a friend: recorded, no reward
--      application_referrals
--
-- The rules every table here keeps (docs/18 §0):
--
...
--      empty client-policy set and 650 checks pg_depend for the views.
...
--      (651 here; Vitest in packages/domain).
...
-- for it: bookings_cancel_cause_check gains 'handed_over' (ADR-0039,
-- restated from 20260924120000 with the one value added). booking_source
-- gained 'offer' in 20260930100000, alone for the reason given there.
--
-- Forward-only. Nothing here edits an earlier migration.
-- =====================================================================

-- ---------------------------------------------------------------------
-- 0 · bookings.cancel_cause += 'handed_over' (ADR-0039, RULE-04 §3.6)
--
...
comment on table staff_unavailability is
  'ADR-0036: days and times a worker has said they cannot work. A hard gate for automated invitations and offer pushes only — never for a manual invite, the worker''s own Accept / apply / take, an open invitation or a confirmed booking. No reason column (Q10). Admin-read; the worker reads and writes through definer RPCs (docs/18 §1). Deleted on GDPR removal.';
...
-- availability.vectors.json holds both (651).
...
begin
  if p_from_date is null or v_to_date < p_from_date or (p_from is null) <> (p_to is null) then
    raise exception 'bad_window' using errcode = '22023',
      hint = 'ADR-0036: a date (or range) and either both times or neither.';
  end if;
  ...
  if p_from is null then
    ...
    if v_to_date = p_from_date then
      if p_to = p_from then
        raise exception 'bad_window' using errcode = '22023',
          hint = 'ADR-0036: from and to are the same time.';
      end if;
...
comment on function public.staff_unavailable(uuid, timestamptz, timestamptz) is
  'ADR-0036: whether the worker has an availability entry overlapping [p_starts, p_ends) — pass the role section''s window (RULE-18). The gate invite_worker(''auto''|''escalation'') and the offer pushes apply (Agent A); never applied to a manual invite or anything the worker does. Invoker: a worker calling it reads no rows.';

-- ---------------------------------------------------------------------
-- §2 · staff_emergency_contacts (ADR-0037)
--
...
comment on table staff_emergency_contacts is
  'ADR-0037: a worker''s optional emergency contact. Office-only worker personal data: never on a client document, never in a client_* view, never in packages/pdf. Admin-read; written through definer RPCs (docs/18 §2). Deleted on GDPR removal.';
...
-- ---------------------------------------------------------------------
-- §3 · profile_change_requests (ADR-0038)
--
...
comment on table profile_change_requests is
  'ADR-0038: a worker''s request to change the name or photo §10.1 locks. One pending per worker per kind; proposed values immutable (GDPR anonymisation excepted); a rejection carries decision_reason, which the worker is shown. Admin-read; written through definer RPCs (docs/18 §3). The machine is profile_change_transitions().';
...
-- changeRequest.vectors.json holds both (Vitest, and 651 through the
...
comment on function public.profile_change_transitions() is
  'The profile_change_requests machine (ADR-0038): pending → approved | rejected | withdrawn; the three outcomes are terminal ("Request again" is a new row). Equal to CHANGE_REQUEST_TRANSITIONS in packages/domain.';
...
begin
  ...
  if new.status is distinct from old.status then
    if not exists (select 1 from profile_change_transitions() t
      ...
      raise exception 'illegal_change_request_transition: % -> %', old.status, new.status
        using errcode = 'P0001',
              hint = 'ADR-0038. The legal edges are listed by profile_change_transitions().';
    end if;
  ...
  if (new.staff_id, new.kind, new.created_at)
    ...
    raise exception 'change_request_immutable' using errcode = 'P0001',
      hint = 'ADR-0038: a request''s worker, kind and creation time never change.';
  end if;
  ...
  if (new.proposed_first_name, new.proposed_last_name, new.proposed_photo_path, new.evidence_path)
    ...
    raise exception 'change_request_immutable' using errcode = 'P0001',
      hint = 'ADR-0038: the proposed values are fixed when the request is made. Withdraw it and ask again.';
  end if;
  return new;
end $$;
...
-- ---------------------------------------------------------------------
-- §4 · shift_offers, shift_offer_notices (ADR-0039)
--
...
comment on table shift_offer_notices is
  'ADR-0039: who has been pushed an offer (OF1), one row per worker per offer, so the hourly rounds are additive and nobody is pushed twice. Admin-read; written by notify_offer_candidates() (Agent A).';
...
comment on function public.shift_offer_transitions() is
  'The shift_offers machine (ADR-0039): open → taken | withdrawn | lapsed | cancelled; the four outcomes are terminal. Equal to SHIFT_OFFER_TRANSITIONS in packages/domain.';
...
comment on function public.shift_offer_mode_transitions() is
  'The one mode change a shift offer has (ADR-0039): office → pool, while open (office_open_offer_to_pool). Equal to SHIFT_OFFER_MODE_TRANSITIONS in packages/domain.';
...
begin
  ...
  if (new.booking_id, new.shift_id, new.offered_by_staff_id, new.target_staff_id,
    ...
    raise exception 'shift_offer_immutable' using errcode = 'P0001',
      hint = 'ADR-0039: an offer''s booking, worker and target never change.';
  end if;
  ...
  if new.mode is distinct from old.mode
    ...
    raise exception 'illegal_shift_offer_mode: % -> %', old.mode, new.mode
      using errcode = 'P0001',
            hint = 'ADR-0039: only office → pool, while the offer is open.';
  end if;
  ...
  if new.status is distinct from old.status then
    if not exists (select 1 from shift_offer_transitions() t
      ...
      raise exception 'illegal_shift_offer_transition: % -> %', old.status, new.status
        using errcode = 'P0001',
              hint = 'ADR-0039. The legal edges are listed by shift_offer_transitions().';
    end if;
...
-- ---------------------------------------------------------------------
-- §5 · staff_referral_codes, application_referrals (ADR-0040)
--
...
comment on table staff_referral_codes is
  'ADR-0040: a worker''s referral code for /apply?ref=. One per worker, never reissued; revoked (revoked_at) on GDPR removal. Admin-read; minted by a definer RPC (docs/18 §5).';
...
comment on table application_referrals is
  'ADR-0040: an application that arrived with a referral code — who referred whom. Kept on GDPR removal of either side (the removed person reads "Deleted account #id"). Admin-read; written only by record_application_referral() (Agent D). The applicant never sees the referrer; the referrer sees a count (Q20).';
...
comment on function public.staff_removed_purge_additions() is
  '§1.7 GDPR removal for the docs/18 additions: deletes availability and the emergency contact, withdraws and anonymises change requests, revokes the referral code, lapses open offers. Fires once, after removed_at is first set. A trigger function: not an RPC.';
