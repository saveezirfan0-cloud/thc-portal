-- =====================================================================
-- Migration 20260930150300 · a referral is recorded for a NEW candidate
--                            only (ADR-0040; security finding #5)
--
-- 20260930140000 let record_application_referral() attach a referral to
...
--   · submit_application_as_caller (20260930140000) — not restated; it
...
--
-- pgTAP 681 (B) asserts a returning-applicant match now records nothing
...
comment on function public.record_application_referral(text, text) is
  'ADR-0040: records who referred an application that arrived through /apply?ref=. Finds the application submit_application() just wrote for this email (same normalisation) — only one that created a new candidate (outcome candidate_created); a returning-applicant match records nothing (security finding #5, 20260930150300). Skips a malformed, unknown, revoked or own code, inserts on conflict do nothing, and never raises. Owner-only: called by submit_application_as_caller(), by no API role.';
