...
-- Band `visa_limit` (20260929150000). Read only for the work_visa and
...
-- audited. ADR-0037 records why the PDF (the later document: "20 hours
...
comment on function public.compliance_set_below_degree_level(uuid, boolean) is
  'D32 / ADR-0037: the office confirms, when it verifies a student''s right to work or term letter, that the course is below degree level — the Student condition is then 10 h a week in term time, not 20. Admin only; International student branch only; audited as rtw.conditions.';
