-- =====================================================================
-- Migration 20260930110100 · offers that can be made again, and the
--                            auto-assign switches read at the insert
--                            (§3.4, §3.6, §10.4; audit 25.09 D33, D9)
--
-- (D3, the additive rounds, is already fixed by 20260928110200: its
-- invite_worker gates on confirmed >= target only. That body is the base
-- restated below, and its target check is unchanged.)
--
...
--   invitation, an office withdrawal, a slot somebody else took
--   (apply_to_shift let a `closed` row through since 20260927161100, but
--   no `cancelled` one, and invite_worker let nothing through). §3.6
...
--   decision a person made. ADR-0037.
--
...
--   writes (20260927160500, the latest), so the two cannot differ (pgTAP
--   661 holds them equal).
--
...
-- (`not_bookable`): since 20260930110000 a candidate or rejected
-- applicant has none, and an absent row must not read as "no gate".
--
-- Bases: booking_transitions() 20260924120000 (the only version);
-- invite_worker() 20260928110200; apply_to_shift() 20260927161100.
-- office_invite_worker() is not restated: it delegates to invite_worker
-- with p_source = 'manual', which is what lets it reopen `person` rows.
-- =====================================================================

-- ---------------------------------------------------------------------
-- 1 · The three reopen edges (ADR-0022 → ADR-0037)
-- ---------------------------------------------------------------------
...
-- 661 holds the two equal for the same booking.
-- ---------------------------------------------------------------------
...
comment on function public.booking_push_payload(uuid) is
  'The N5 values for one booking — exactly what queue_booking_push() writes (20260927160500): role, event, date, dateTime (Europe/London), the BASE rate, window. Used where an N5 needs its own key: a reopened invitation (20260930110100).';
...
-- ---------------------------------------------------------------------
-- 4 · invite_worker — 20260928110200 but for D9 (switches at the insert)
--     and D33 (reopen), both marked
-- ---------------------------------------------------------------------
...
begin
  ...
  -- No candidate row (20260930110100): not a worker (a candidate, a
  -- rejected applicant), left or removed. It must not read as "no gate
  -- applies".
  if not found then
  ...
  -- The live re-check. Seats, not the invitation target: Radar offers a
  -- shift while confirmed (or checked in) < headcount, and applications
  -- close at the same point (close_filled_role_applications,
  -- 20260930110200).
  select * into v_fill from shift_fill(p_shift);
  ...
  -- A LIVE booking blocks a second one. An ended one is REOPENED rather
  -- than a second one inserted — (shift_id, staff_id) is unique — unless
  -- it is a self-cancel (RULE-04), an event cancellation or a GDPR
  -- removal, or carries history: §3.6 reserves the permanent bar for
  -- self-cancel, and §10.4 tells the worker "You can still apply for this
  -- shift on Radar later if it's open". 20260927161100 reopened `closed`
  -- rows only; an office withdrawal or a 12:05 release is reopened too
  -- since 20260930110100 (D33) — the worker's own application is a
  -- person's decision, which booking_reopenable_by() lets through.
...
comment on function public.apply_to_shift(uuid, uuid) is
  '§10.4 Radar self-application. The caller''s OWN row (staff_writer), or a worker named by the office from the event board (§3.3); any other caller naming another worker is refused. Refuses event_cancelled · shift_started · not_bookable (no candidate row) · the gate by name · full (confirmed-or-worked >= headcount) · already_has_booking (a live row, or a self-cancel / event-cancelled / GDPR row, or one with history). Any other ended row is reopened as applied (20260930110100). Every refusal is a reason, never an exception.';
