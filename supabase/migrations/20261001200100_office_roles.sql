...
-- (ADR-0056; ADR-0055 "Proposal, not built: finer office permissions";
...
--
--   users    — /users: the account functions of 20261001200000 and the
...
--
-- What the scheduler can STILL read is stated in ADR-0056 ("Residual
...
-- re-created with every check their 20261001200000 body had, in the same
...
comment on column profiles.office_role is
  'Back Office permission level (ADR-0056): owner = everything; manager = everything but Users & access and Settings; scheduler = also no money. Set for admin rows only (check constraint); changed only through admin_set_office_role().';
...
-- auth.users.banned_until (ADR-0055 "Known limit").
-- ---------------------------------------------------------------------
...
comment on function public.office_can(text) is
  'ADR-0056: may the signed-in Back Office login use ''users'' | ''settings'' | ''finance''? owner: all three; manager: finance; scheduler: none. False for any other session and any other permission name.';
...
--
-- Each body below is 20261001200000's, check for check; the additions are
-- marked "ADR-0056".
-- ---------------------------------------------------------------------
...
comment on function public.admin_accounts(app_role) is
  '/users: every login with its role, office role, client, last sign-in and whether it is switched off. Back Office owners only (office_can(''users''), ADR-0056).';

-- 3b · admin_register_account gains p_office_role.
--
-- Two signatures, deliberately. The six-argument one is the real one and
-- takes the office role explicitly. The five-argument one — 20261001200000's
...
begin
  ...
  -- ADR-0056: an office role belongs to an admin login and to nothing else.
  ...
  -- ADR-0056: the office role is set on a NEW login only. A re-invite
...
comment on function public.admin_register_account(uuid, app_role, text, uuid, text, office_role) is
  '/users Invite: gives a GoTrue login minted by the office''s service key its profile, office role (new admin logins only) and app_metadata.role (admin or client — never staff). Refuses to change the kind of an existing login. Back Office owners only (ADR-0056); audited as account.invited / account.reinvited.';
...
comment on function public.admin_register_account(uuid, app_role, text, uuid, text) is
  '20261001200000''s signature, kept: calls the six-argument admin_register_account with office role ''manager'' for an admin login (ADR-0056''s default). Every check is the six-argument function''s.';
...
comment on function public.admin_login_lookup(text) is
  '/users Invite: whether an address already has a login, its kind, client and whether it has ever been signed in to — asked before any token is minted. Back Office owners only (ADR-0056).';
...
begin
  ...
  -- ADR-0056: nor lose the last working owner — the only role that can
...
comment on function public.admin_set_login_disabled(uuid, boolean, text) is
  '/users: switch an office or client login off (sign-in refused, sessions ended) or back on. Refuses staff (use Block), the caller''s own login, the last working admin and the last working owner. Back Office owners only (ADR-0056); audited.';
...
-- codes off these rows (ADR-0056 "Residual gaps"). The predicate names
...
begin
  ...
  if tg_op = 'UPDATE' and new.role_id is not distinct from old.role_id then
    if new.pay_rate is distinct from old.pay_rate
      ...
      raise exception 'rates_need_finance' using errcode = '42501',
        detail = 'Changing a pay or charge rate needs a manager (ADR-0056).';
    end if;
    return new;
  end if;
  ...
  if new.pay_rate is distinct from v_pay or new.charge_rate is distinct from v_charge then
    raise exception 'rates_need_finance' using errcode = '42501',
      detail = 'A scheduler''s role section carries the role''s pay rate and the client''s rate-card charge (ADR-0056).';
  end if;
  return new;
end;
$$;
