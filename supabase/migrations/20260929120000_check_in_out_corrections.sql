-- =====================================================================
-- Migration 20260929120000 · the day of the shift, corrected
--                            (§3.3, §5.1, §5.2, §5.2b, §9.5, §9.6,
--                             RULE-01 / 02 / 14, ADR-0032)
--
-- Audit 25.09.2026, fix round WP-C. Each numbered block names the defect.
--
...
--       `left_early` violation (§9.5). ADR-0032: a press more than 15
...
--       stays that fix until a manager resolves it (ADR-0032).
--
...
--
--   D4  "Get back" on the event board is get_back(), which resolves the
--       open No-show through resolve_violation() — the same action as
--       Resolve in the §9.5 log. It registers the arrival (a check log),
--       moves the booking to `worked` and reclassifies the entry to Late,
--       so payable_shifts_v pays the shift, the worker's screen unlocks
--       and booking_tick() has nothing left to re-raise.
--
...
--       so it settles instead of staying Pending.
--
...
--
-- Restated from their latest definitions:
...
comment on function check_out_decision is
  'Pure §5.1 check-out gate. Mirrored by checkOutDecision() in packages/domain/pay.ts; both are held to pay.vectors.json. leftEarly = pressed more than 15 min before the end (D5); an off-site fix more than 30 min before both the press and the end raises no_checkout for review (D15, ADR-0032).';
...
begin
  ...
  -- RULE-02, either trigger, and D15's stale fix.
...
-- 6 · get_back (D4, §3.3)
--
-- The event board's "Get back": the same action as Resolve on the No-show
-- in the §9.5 log (the scope says so twice), so it IS that action. The
-- board has no note field; the note records where it came from.
-- ---------------------------------------------------------------------
create or replace function get_back(
  p_booking       uuid,
  p_arrived_at    timestamptz default null,
  p_actual_finish timestamptz default null
) returns jsonb language plpgsql set search_path = public, extensions as $$
...
begin
  ...
  if v_id is null then
    raise exception 'no_open_no_show' using errcode = 'P0001';
  end if;

  return resolve_violation(v_id, 'Get back — arrival registered on the event board.',
                           p_actual_finish, p_arrived_at);
end $$;

comment on function get_back(uuid, timestamptz, timestamptz) is
  '§3.3 "Get back": resolves the booking''s open No-show through resolve_violation() — registers the arrival, moves the booking to worked, reclassifies to Late (D4).';

revoke execute on function get_back(uuid, timestamptz, timestamptz) from public, anon;
grant  execute on function get_back(uuid, timestamptz, timestamptz) to authenticated;

-- ---------------------------------------------------------------------
-- 7 · The automatic client qualification on a CLOSED clean shift (D37)
--
