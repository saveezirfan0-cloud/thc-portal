-- =====================================================================
-- Migration 20260930150000 · offer up a shift — review fixes
--   (ADR-0039 "Amendment · review fixes"; QA + security review of
--   20260930110100_shift_offers.sql)
--
-- Every function below is restated IN FULL from its latest definition
-- (20260930110100 — nothing later redefines any of them) with the one
...
--       refused with 20260930120000's shape: unknown_staff /
...
--
-- Forward-only. References only 20260930110100 and earlier objects.
-- =====================================================================
...
comment on function public.queue_offer_notice(text, uuid, uuid) is
  'ADR-0039: queues one of OF1–OF6 for an offer under its register key (OFn:offer:<id>; OF1:offer:<offer>:<staff>; OF5:booking:<booking> — once per booking however often cover is asked) with exactly the payload keys packages/notifications renders, plus offerId/shiftId/eventId (and bookingId) for tooling. Push dates as N5, OF5 as E10; the base rate only; a blank note is "—". Internal: called by the offer functions, never an RPC.';
...
comment on function public.withdraw_shift_offer(uuid) is
  'ADR-0039: the worker withdraws their own open offer (a pool offer or a cover request). Caller by staff_caller(): unknown_staff / account_closed / not_editable raise P0001. Under the section lock, so it cannot race a take: offer_not_open once somebody has it.';
...
comment on function public.office_decline_cover(uuid, text) is
  'ADR-0039: the office closes a worker''s cover request (open → cancelled) and queues OF6; the worker stays booked. The note is the office''s own record (closed_reason), never sent to the worker and never copied into audit_log (has_note only). Admin only; audited.';
...
comment on function public.lapse_shift_offers(timestamptz) is
  'ADR-0039: closes every open offer past its expiry (open → lapsed, closed_reason expired) and queues OF3 — "you''re still booked" — for each that had gone to other workers, unless the section has already started. The worker stays confirmed. Service role; the auto-staffing hourly run calls it first.';

-- ---------------------------------------------------------------------
-- Grants, as 20260930110100 set them.
-- ---------------------------------------------------------------------
