-- =====================================================================
-- Migration 20260930120200 · Request a change — the worker's side
--   docs/18-staff-features-plan.md §3 (Phase 1, Agent B · staff-pwa)
--   ADR-0038 (proposed — awaiting THC) · Q13, Q14, Q21 · §8 RC1
--
...
-- (Phase 0, 20260930100100: admin_read only, state guard, shape CHECKs,
...
--          the bucket has no worker policy and gets none (docs/18 §3).
...
comment on function public.request_profile_change(text, text, text, text, text, text) is
  'ADR-0038: the calling worker asks the office to change their locked name (first/last + evidence in documents/<id>/change-requests/) or photo (a fresh object in photos/<id>/). One pending per kind; the uploaded object must exist; a name equal to the current one is refused. Queues RC1 to admin@ in the same transaction. Never writes staff.';
...
begin
  ...
  if v_req_status <> 'pending' then
    raise exception 'not_pending' using errcode = 'P0001',
      hint = 'ADR-0038: the office has already decided it.';
  end if;
...
comment on function public.withdraw_profile_change(uuid) is
  'ADR-0038: the calling worker withdraws their own pending change request. not_found for another worker''s id; not_pending once the office has decided.';
...
comment on function public.my_profile_change_requests() is
  'ADR-0038: the calling worker''s own change requests, newest first, with the office''s reason on a rejection (shown to the worker). Never returns decided_by, previous_value or evidence_path.';
